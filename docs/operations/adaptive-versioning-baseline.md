...
| Visits GET lost after adaptation | nothing tracked functions | route capability diff (`capabilities.lost`) in the report and brief; **enforced (AV06)**: `MaxFinalizationCoordinator.finalize` compares the draft before the adaptation run with the result and returns `NEEDS_EDIT` listing every lost `METHOD /path` (2 repair rounds, then the run fails and the draft is unchanged) — `apps/api/src/yleum_api/services/versioning_capabilities.py` |
...
- Restoration candidates are a `verification` capacity workload (own CPU budget `CELL_VERIFICATION_CPU_CORES`, real-free memory/disk, 8 GiB disk) and no longer compete with the runtime ledger; waking the draft itself still needs 4.2 runtime cores.
  **`CELL_VERIFICATION_CPU_CORES` is a pool, and it must be at least one whole candidate.** A candidate is built from the public profile (`production_manager`), so with the shipped public limits it asks **1.45 cores** — a budget below that refuses every restoration on any host at any load, not just a busy one. Host cores and load are deliberately not consulted here, so free CPU is no defence. The shipped default (2.0) admits exactly one candidate and refuses a second, i.e. **one restoration check at a time per host**; raise it only if you accept that many verification cells running beside the ordinary ones. Since `ac66b40f` a too-small pool is reported as `verification_budget_too_small` with both numbers, separately from the ordinary busy-pool refusal — but the setting still has to be right. On 23.09.2026 both cell hosts were configured with 1.0 and no rollback could run at all.
