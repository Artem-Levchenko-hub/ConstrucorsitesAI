...
## 7. Как прошло окно (25.09.2026 20:10–20:22 UTC, голова 655e2ef8)

Всё по разделу 4 с поправками порядка (merge и `uv sync` до остановки службы, пересборка venv, перенос drop-in, без `--remove-orphans`). Ячейки: список по `label=omnia.managed` до и после совпал (16 на core). Тома `full_*` на месте. Резервная копия с новыми именами прошла (53 с, MinIO OK).

**Ловушка, которой не было в инвентаре — подсеть compose-сети.** Ключ сети `omnia-prod → yleum-prod` заставил compose создать новую сеть `full_yleum-prod`, и Docker выдал ей следующую свободную подсеть (172.21.0.0/16 вместо 172.19.0.0/16 у `full_omnia-prod`). Хостовый PostgreSQL (`10.10.0.1:5432`) пускает контейнеры платформы по правилу ufw и строке `pg_hba.conf` **с конкретной подсетью**, поэтому api после пересоздания не мог подключиться к базе (asyncpg TimeoutError, контейнер перезапускался, воркеры не стартовали как зависимость) — простой api ~8 минут (20:11–20:19). Починка: `ufw allow from 172.21.0.0/16 to 10.10.0.1 port 5432 proto tcp` + строка `host omnia omnia 172.21.0.0/16 scram-sha-256` в pg_hba + reload. Правило: **при любом пересоздании compose-сети платформы сверить подсеть (`docker network inspect full_yleum-prod`) с ufw и pg_hba** — старые правила для 172.19.0.0/16 оставлены до уборки старой сети.

**После смены подсети перезапускать ВСЕ контейнеры, которые ходят в хостовую базу, а не только те,
чей код менялся.** 25.09 правила ufw и pg_hba были добавлены в 20:19, а шлюз моделей упал при старте
в 20:12:38, записал `startup.postgres_unavailable`, перешёл в режим деградации и **не переподключался**.
Health он при этом отдавал 200, поэтому никто не заметил: десять часов каждый вызов модели оплачивался
у провайдера, а списание падало с «DB pool not initialized» — в журнале расхода за это время ноль
записей (последняя 25.09 18:37 UTC). Лечение — пересоздать контейнер: `docker compose up -d --no-build
--no-deps --force-recreate gateway`. Сценарий выкатки теперь проверяет это на каждой волне.

**Остановленный `omnia-prod-postgres` — не мусор.** Это контейнер отключённого профиля `compose-postgres` с томом `full_postgres-data`, путь отката базы платформы на хостовый PostgreSQL (см. `infra/max-k3s/migrate/50-platform-db-to-host.sh`). Compose его не трогает и не переименовывает; сносить только шагом `retire` того скрипта.

Ревизия: оркестраторы стартовали с кода 655e2ef8 (умолчания адресов платформы из de90b6ee), поэтому им выставлен `OMNIA_RELEASE_SHA=655e2ef8` и ожидание `PRODUCTION_EXPECTED_ORCHESTRATOR_RELEASE_SHA=655e2ef8`; api/воркеры остались на образе 1eba74c2 (код api не менялся), web — ecb31a3c. Старые остановленные контейнеры `omnia-prod-*` и пустая сеть `full_omnia-prod` убираются отдельно после отстоя.
