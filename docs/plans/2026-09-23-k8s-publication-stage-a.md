...
- **Сделано 23.09 (вечер):** DNS `*.dev2` заведён владельцем, commerce включён в реестре
  (`ORCHESTRATOR_HOSTS`), API `8ef441a4`. Живая проверка: новый проект → ячейка на commerce →
  генерация 23 мин → превью с сертификатом на `dev2` (200 через preview-session) → удаление
  проекта на commerce. Публикация с commerce проверена по частям (доступ к кластеру, реестр,
  артефакты для подов) — сквозная требует подключённого MAX-бота. Отчёт H208.
