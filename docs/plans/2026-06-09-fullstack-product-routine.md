...
- [~] A5 — **Миграции БД на provision** (частично DONE). FIX: добавлен `db:push` script в package.json (entrypoint звал `pnpm db:push` → «Command not found» → 0 таблиц). Теперь provision СОЗДАЁТ таблицы (commit `7d5365c`, задеплоено: push+VPS pull). Тест-апп разблокирован вручную (raw SQL). НО для свежих аппов остаются 2 блокера ниже (A5b, A6a) — без них регистрация всё равно падает.
...
- [ ] A6a — **AI выбрасывает auth-колонки** (`passwordHash`,`role`) при переписывании `schema.ts` под свою сущность → signup/login падают (шаблон корректен; AI дропает). Мандат в fullstack-промпте: users/accounts/sessions/verificationTokens — managed auth-таблицы, копировать ВЕРБАТИМ; + детерминированный guard в пайплайне (ре-инъекция auth-колонок).
