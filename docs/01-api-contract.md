...
| `POST` | `/api/payments/subscription` | `{plan_code: "pro" \| "business", idempotency_key}` | Создаёт pending-подписку и redirect-платёж первой покупки |
...
на этом этапе намеренно не включено и требует отдельного согласия.
