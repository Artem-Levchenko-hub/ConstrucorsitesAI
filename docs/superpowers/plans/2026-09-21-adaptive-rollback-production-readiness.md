...
- Первый документальный checkpoint: `2fc1b3bea50cc30e3356d193c9ad8f17db7f94c5`, pushed branch `origin/codex/rollback-audit-20260921`. Перед этим продолжением clean HEAD совпал с remote branch по `git ls-remote` и explicit remote-tracking fetch, ahead/behind `0/0`; `origin/main` остаётся `0676e0be`. `@{upstream}` shorthand не разрешался из-за main-only fetch refspec, но configured remote/merge и точный remote SHA проверены отдельно. Branch push не является доказательством merge в main или production deploy; эти delivery этапы остаются отдельными.
- **Текущий hard safety stop:** после failed v8 нельзя запускать v7→v5 restore, обычный runtime restart/start, preview-session repair или adaptive. Сначала выполнить recovery gate §0.3/T20; текущие source/runtime/DB authorities расходятся. Прежний empty exact PASS остаётся исторически действительным.
...
## 0. Свежая live-матрица: первый срез 18:05:28 МСК и продолжение с hard stop после 18:41:54 МСК 21.09.2026

Первый фактический срез был подготовлен для commit/push до 18:11 МСК и сохранён commit `2fc1b3bea50cc30e3356d193c9ad8f17db7f94c5`. **Автоматический empty exact restore прошёл E2E.** Последующее synthetic CRUD тоже прошло, но compatible successor generation завершилась failed с изменённой схемой БД, stale source и остановленным runtime. Продолжение ниже фиксирует hard stop: compatible restore, explicit adaptive и fault/restart/settlement matrix **BLOCKED из соображений сохранности**, а не объявлены покрытыми. Задания T01–T23 — remediation/automation plan, не утверждение о внесённых исправлениях.

QA project: `361b3326-97b4-4c73-94d5-c8d277a69dc6`. Platform owner: `f5a028d8-eed4-4fe7-9452-5a056174fb39`; второй QA owner: `d18f1fa0-d0a8-478c-b2d2-e5e66889a939`. Использованы только новые синтетические QA-объекты. Evidence первого среза — live UI и корреляция backend SQL/logs по UUID; отдельного screenshot bundle не было. Для продолжения сохранены четыре локальных raw artifacts, перечисленные в §0.2; они не являются уже опубликованным полным evidence bundle и не должны попасть в Git с секретами.
...
| LIVE-18 | Compatible restore с synthetic rows, hidden values/delete | BLOCKED hard safety stop после failed successor v8 | CRUD до successor проверено отдельно LIVE-21; restore v7→v5 не запускался | Новый restore запрещён до source/runtime/DB reconciliation §0.3; empty PASS остаётся действительным |
| LIVE-19 | Incompatible explicit adaptive restore до completed | BLOCKED hard safety stop; не запускался | В текущем QA project adaptation run отсутствует | Не обходить failure обычной генерации новым adaptive run; прежний successful v8 от 19.09 — отдельный baseline |
| LIVE-20 | Post-activation signed two-owner, restart, adaptive receipt/settlement, 5 faults | BLOCKED hard safety stop; не выполнено | Exact empty applied version/snapshot подтверждены LIVE-16; protected adaptive activation отсутствует | Platform owner2 404 не заменяет hostile business-row proof; cached preview не заменяет running runtime |
...
### 0.1. Архивный handoff первого checkpoint — superseded продолжением §0.2/§0.3

Следующий перечень сохраняет историю передачи после successful empty restore. После начала его исполнения подтверждены empty outcome и synthetic CRUD; дальнейшие compatible/adaptive пункты не выполнены и теперь заблокированы. **Не выполнять архивный перечень как текущий runbook после hard stop.** Актуальная точка остановки — §0.3.
...
### 0.2. Продолжение: synthetic CRUD PASS, failed compatible successor и hard safety stop

| Case ID | Фактический шаг | Статус | Exact evidence | Граница вывода |
|---|---|---|---|---|
| LIVE-21 | Synthetic create/read/API update/UI update | PASS до successor generation | client `bce38a28-94dd-46b2-9826-17112aaa20dd`; create 201, GET 200, PATCH 200; UI name `QA Synthetic Compatible R1 UI`, toast `Клиент обновлён`, list count=1, status `in_progress` | UI и API работали на этом этапе; delete и hostile signed B не выполнялись |
| LIVE-22 | Независимый baseline row/history witness | PASS на момент capture | clients/history `1/3`; row digest `11abcfb656d1983250e8f5276bf339c8`; history digest `c2d76fbebb8269acdd4be3c10c6b0bcc` | 32-hex значения — записанные opaque digest, не называются SHA-256 без подтверждения алгоритма |
| LIVE-23 | Compatible successor generation с nullable note | FAIL | run `48e60d8e-9388-47cd-80b2-3af25b61f655`, assistant `d295484f-b100-4456-a576-c9c1ad343b1b`, source message `ecce8aa2-c29d-43ed-8376-5e8ab814698b`, draft #8 `30003341-7976-4577-b306-5da015863998`; start `15:16:24.705Z`, failed `15:41:24.713661Z` | UI 25m03/65 steps; final_build успел завершиться, timeout на runtime_probe; это не попытка compatible restoration |
| LIVE-24 / NEW-30 | Deadline/lease terminalization | FAIL диагностики/терминального перехода | UI: `activity already timed_out; cannot finish as cancelled`; lease `adf666cb-fcd0-5d69-b2b0-cfdddb439576` timed_out, phase runtime_probe; underlying diagnostic `generation deadline exceeded stage=edit phase=runtime_probe` | Повторная попытка cancelled скрывает первоначальный timeout; stage=edit при runtime_probe здесь относится к обычной successor generation, не доказательство отмены уже внедрённых adaptive stage budgets |
| LIVE-25 / NEW-27 continued | Additive migration на текущей DB при failed version | FAIL atomicity; row preservation PASS только до stop | `drizzle/0002_qa_clients_note.sql`, 305 bytes, SHA-256 `196fe2645ca45ff9c365e6cc66ef7987c2c3d7b413fb30d9af89109b33e00468`; SQL `ALTER TABLE qa_clients ADD COLUMN IF NOT EXISTS note text;` | Миграция применена во время verification; note nullable text/no default, existing row NULL. 0001 неизменна: SHA-256 `3b52f2ec49ba2c1c40747942b2781a8d800b224a7ac8159f84400ae4c4fa5158`; clients/history 1/3, оба baseline digest совпали до teardown |
| LIVE-26 / NEW-31 | Current version versus editable source после failed run | FAIL / P0 | #8 failed/is_current=false; #7 current snapshot `4d1089a5-ef13-5dcd-b313-7f0cf4b88c04`, commit `6371b71b9b3ca83ba31119bbc5b4bfc996cc4c94`; editable workspace 45/46 files match #7 и **46/46 match old v6** | Единственный drift — `src/app/page.tsx`; config/0001/schema/CRUD routes exact; generated 0002/test removed. Exact restore переключил machine code+DB volumes, но agent workspace не синхронизирован; failure cleanup сохранил stale pre-restore v6 |
| LIVE-27 / NEW-32 | После release открыть новый preview | FAIL / P0 runtime authority | Все observed gates idle: generations/restorations/leases=0; workspace ready, fence17/version18; release operation prefix `98582672` completed; новая preview-session → 409 `draft runtime is not running` | App и project-postgres не запущены. Старый visible tab/reload — cached либо краткое pre-teardown evidence, не актуальное serving proof |
| LIVE-28 | Сохранность и identity отключённого active DB volume | PASS физического наличия/identity; SQL witness BLOCKED frozen state | Registry active DB `omnia-machine-891ed2449b00458babf49f23f194aaab-db-4ecaa09ac717487dbe39553062a537e5`; registry sealed `15:41:54Z`; raw content manifest SHA-256 `99d820b37c0f9e85de5f5b3d71b3441a42201a63ce92fa0c26ef538f9fa537ba`, max mtime `15:41:31Z` | Том detached, доступен, не объявляется потерянным. Raw string hits не доказывают строки/транзакционную сохранность после stop; exact post-stop SQL digest не получен |
| LIVE-29 | Compatible restore v7→v5 | BLOCKED hard safety stop; не выполнялся | `verify_source_inventory` сравнил бы current snapshot с stale workspace; actual новая operation не создавалась | Current code предусматривает `needs_changes` до candidate. Не обходить этот fence и не считать прогнозированный отказ выполненным тестом |
| LIVE-30 | Incompatible/adaptive, delete, signed two-owner rows, restart, settlement, faults | BLOCKED hard safety stop; не выполнялись | Источник/current runtime/DB authority не согласованы | Сначала gate §0.3; ни один незапущенный сценарий не помечается PASS |

History baseline этого synthetic client, UTC:

| History ID | Timestamp | Event |
|---|---|---|
| `0471b10e-2656-46ee-b373-6da70857e1f3` | `2026-09-21T15:14:15.430Z` | created |
| `2ddb3d27-be67-4404-9c5b-58ceed512361` | `2026-09-21T15:14:16.466Z` | API updated |
| `bf9c5228-d0f2-4104-aedb-cfdfa624981f` | `2026-09-21T15:14:37.770Z` | UI updated |
...
Read-only raw scan активного DB тома: exact synthetic name 12 hits, phone 16, `qa_clients` 39, `note` 28. Legacy app-postgres volume — zero hits; cell postgres не содержит QA tables. Это подтверждает выбор **registry-bound active volume**, а не старого/служебного PostgreSQL. Число совпадений в WAL/страницах не равно количеству строк; SQL после stop не выполнялся, чтобы не менять frozen original volume.

Failed run reported usage: 1 487 081 input / 27 242 output tokens, recorded charge/cost 123.8409 RUB / 0.821500 USD. Wallet settlement receipt отдельно не проверен; нельзя заявлять ровно одно списание, refund или отсутствие дубликата только из этих полей. Проверка — T08/T23, только по scoped QA run/account.

Локальные raw evidence paths на компьютере текущего расследования:

- `%TEMP%\omnia-current-v7-snapshot.json` — authoritative #7 source snapshot.
- `%TEMP%\omnia-v6-snapshot.json` — pre-restore v6 snapshot для 46/46 comparison.
- `%TEMP%\omnia-v7-volume-manifest.json` — read-only volume content manifest.
- `%TEMP%\omnia-live-page-after-failed-v8.raw.tsx` — actual stale page source.

`%TEMP%` должен разрешаться на исходном компьютере, а не автоматически на новом. Перед переносом — открыть только разрешённые synthetic artifacts, проверить redaction/hashes/private permissions, сформировать manifest. Эти файлы не коммитить вместе с планом и не предполагать, что после перехода на другой компьютер они уже доступны.

При подготовке второго checkpoint наличие и bytes SHA-256 этих четырёх файлов проверены локально; здесь `%TEMP%` разрешился в `D:\UserData\Temp`. Проверялись метаданные/hash, не заявляется проведённая redaction содержимого:

| Файл в `D:\UserData\Temp` | Bytes | SHA-256 именно файла |
|---|---:|---|
| `omnia-current-v7-snapshot.json` | 296882 | `0d0417a06ed66f00cde24b7d9bb13c9cf3cc0b0e4e9208e267329de772c6802a` |
| `omnia-v6-snapshot.json` | 297358 | `f5f2eef8a4fe98b8f5eb13114139e00c1b2785f42e8e2c349a82b8c564da7958` |
| `omnia-v7-volume-manifest.json` | 3721143 | `b1e28a355ab2c73e30cb56b29a714215f85561a54e5fe2fd3ea2e98d4fbeea30` |
| `omnia-live-page-after-failed-v8.raw.tsx` | 44305 | `7fbf2694c868bdd9655b2034dcb029c9dc04f96d51e056bc544e50ca69a2c4c4` |

JSON-file hash manifest `b1e28a...` отличается от его semantic volume-content digest `99d820...` и registry artifact digest `cdd597...`: это три разные сущности. Проверка целостности должна сохранять тип digest и алгоритм canonicalization, не сравнивать их как один hash.

### 0.3. Актуальный stopping point: hard stop и recovery gate RC0–RC7

**Запрещено до gate:** новый v7→v5 restore, обычный runtime restart/start, попытка «исправить» preview-session, новая generation/adaptive, удаление candidate/active DB volume, migration replay на original DB, автоматическое переключение на legacy app-postgres. Все actions этого subsection — будущий контролируемый recovery, в текущем continuation они не выполнены.

| Gate | Конкретное действие / invariant | Требуемое evidence / stop condition |
|---|---|---|
| RC0 Freeze / identity | Fresh Git/server preflight; read-only current #7 snapshot/commit, #8 failed run, actual workspace/fence/registry and active-volume names. Scope только project `361b3326-97b4-4c73-94d5-c8d277a69dc6`. Сохранить aggregate idle; запрещать новые actions до release gate | Snapshot #7 `4d1089a5-ef13-5dcd-b313-7f0cf4b88c04`, commit `6371b71b9b3ca83ba31119bbc5b4bfc996cc4c94`, restored operation/binding и exact DB volume согласованы. Любая foreign/missing binding → stop |
| RC1 Read-only volume binding | Получить полное active code volume из registry; inspect labels/owner/workspace, controller incarnation, DB system identity, source artifact manifest. Original DB mount только `readonly`, helper `--network none`, никаких entrypoint/init/upgrade на нём | Полное code/DB pair, source commit и manifest digest. Legacy app-postgres/cell postgres не разрешены как fallback; raw hits остаются вспомогательными |
| RC2 Independent post-stop SQL witness | На **синтетическом** source volume, после identity proof, снять consistent isolated clone через read-only source mount. Original не запускать. Поднять PostgreSQL той же major/image только на disposable clone; startup/WAL recovery пишет только clone. SQL connection `READ ONLY`, проверить schema/note, row ID/owner/name/status и три exact history events | Clients/history 1/3, before-stop row/history digests совпали по тому же canonical query либо объяснённому allowlist, note nullable/no default/NULL. Получить реальный post-stop SQL witness; не выдавать raw scan или cloned filesystem hash за него |
| RC3 Safe source reconciliation | Новый durable recovery intent с fresh fence, expected current snapshot #7, expected observed stale page hash и bound active volumes. Только после RC2 заменить `src/app/page.tsx` точными bytes #7 из trusted snapshot; сначала CAS observed hash, затем atomic code write. Сверить весь 46-file inventory в editable workspace и current code volume. Никаких DB mounts на запись для этого шага | Page expected hash `d80652ddc0e6168a13192ef72273cc64b11d645b701d34876303df111d5e50cf`; все 46 paths/bytes match #7. При drift другого файла/новом run/fence — stop, не blanket restore. Preserve frozen stale evidence |
| RC4 Current v7 runtime start | Только после RC0–RC3 controlled recovery стартует **current #7/v7**, существующую registry-bound DB, без создания новой DB и без повторного исполнения исторических migrations. Подтвердить image/major/start command и applied migration journal против clone witness; если safe start без replay не доказан, stop и исправить recovery path | Running app+project-postgres, exact bound DB volume, current source commit, no replacement/DDL replay/new empty volume. `workspace ready` alone не PASS |
| RC5 Signed runtime/data acceptance | Получить **новые** signed A/B sessions. Fresh A GET читает одну baseline строку; B не читает/меняет/удаляет её; anonymous denied. Отдельная temporary QA row проходит create/update/reload/delete; baseline row и три history events без изменений | Fresh uncached HTTP statuses + independent SQL witness. Новые events temporary row отделить по row/run ID; baseline digest query не должен зависеть от допустимых новых rows |
| RC6 Recovery durable completion | Reconcile source/runtime/registry/API projection, terminal lease/release journals, exact code inventory, complete current serving proof. Scoped quota/wallet receipt проверить отдельно; не потреблять новую generation quota за recovery | One current #7, #8 remains failed, no duplicate version/settlement, correct operational recovery status, signed preview works; reopen admission только после all PASS |
| RC7 Resume acceptance | Сначала controlled compatible successor повтор с новым run ID и witness, после успешного fresh version — compatible restore. Только затем incompatibility → explicit adaptive и остальные матрицы | Все новые IDs/SHAs/checkpoints в evidence; stop вновь при source/runtime/DB split. Прежний empty exact PASS не переделывать и не стирать |

RC3 не разрешает просто вызвать старый `_restore_project_cell_source` через timed-out handle: нужно новое admission/recovery ownership. RC4 — исключение из hard stop только после доказанных RC0–RC3; это не blanket authorization на runtime restart. Поддержать recoverable interruption после каждого gate (T20/T21), не откатывать original DB при неудаче.

Передача другому компьютеру: начать с read-only RC0, получить artifacts §0.2 с исходного компьютера через разрешённый private канал, затем выполнять T20 gate. Если доступна только копия репозитория без registry/volume witness, продолжать tests/remediation в disposable среде, а live recovery оставить BLOCKED. Не нажимать старую вкладку preview как способ запуска: она не свидетельствует о работающем runtime.
...
| Admission aggregates до live-прогона и после failed v8 | Изначально все 0; в последнем continuation observed generation/restoration/leases=0, workspace ready fence17/version18, release completed | Idle не означает source/runtime/DB consistency: app/DB stopped и editable source stale. Для restart действует RC0–RC7, не только нулевые счётчики |
...
| NEW-30 live application P0 | Deadline + activity finalization | `activity already timed_out; cannot finish as cancelled` скрывает timeout runtime_probe | Deadline наступил после final_build; cleanup пытается перевести уже terminal timed_out lease в cancelled | LIVE-23/24, run `48e60d8e-9388-47cd-80b2-3af25b61f655`, lease `adf666cb-fcd0-5d69-b2b0-cfdddb439576` | Неоднозначный исход и пропуск source recovery; дорогой failed run | T22 | Первичный timeout сохраняется; один terminal state; independent durable recovery завершён либо явно BLOCKED |
| NEW-31 live application P0 | Exact activation / editable source lineage | Current #7, machine pair #7, editable workspace целиком old v6; 45/46 совпадение с #7 | Machine volume switch не синхронизировал agent workspace; terminal guard в agent_finalization.py:176 rethrows до source fallback; ready/idle preparation не reconcile | LIVE-26, page hashes и 46/46 v6 match; code guard source lines в §0.2 | Следующая generation стартует от stale UI, restore fail-closed; source truth split | T20/T21/T22 | Current snapshot, active code volume и editable 46-file inventory bound и равны до admission |
| NEW-32 live application P0 | Current runtime / volume authority | workspace ready/idle, app+project-postgres stopped, fresh preview 409 | Release завершён без usable serving runtime; detached active DB остаётся registry-bound; ready resources не является app readiness | LIVE-27/28, release prefix `98582672`, active volume и hashes | Неверный wake может выбрать legacy пустую БД либо уничтожить synthetic state | T20/T23 | Start current #7 на exact retained DB после RC0–RC3; fresh signed A/B and SQL witness PASS |
| NEW-27 continuation P0 | Failed compatible DDL | #8 failed, nullable note применена, migration source удалён cleanup | Реальный additive effect в source DB пережил неуспешную generation; baseline rows/history не изменились до stop | LIVE-25; 0002 SHA, 0001 immutable SHA, 1/3 и digests | Schema не соответствует successful source history; после stop SQL сохранность ещё не доказана | T19/T20 | Post-stop SQL witness + durable schema-effect record; не downgrade note ради версии |
| U05 unproven | Paid usage/settlement | Failed run имеет 123.8409 RUB / 0.821500 USD recorded cost | Usage известен, отдельный wallet settlement receipt не проверен | run `48e60d8e-9388-47cd-80b2-3af25b61f655`, assistant ID в §0.2 | Нельзя объявлять exactly-once/refund/no-charge по одному cost field | T08 | Scoped ledger/usage receipts, отсутствие duplicate logical charge; result явно проверен |
...
LIVE-16 empty exact PASS (historical) → LIVE-21 CRUD PASS → failed v8 HARD STOP
    → RC0/RC1 authority bind → RC2 SQL witness → RC3 source sync
    → RC4 controlled current-v7 start → RC5/RC6 signed proof → resume acceptance
T20 recovery + T21 workspace lineage + T22 terminal recovery + T23 runtime authority
    → prevent recurrence of failed-v8 source/runtime/DB split
...
- [ ] 1: real volume fixture воспроизводит active restored DB с 1 client/3 history/note NULL, отдельную пустую legacy DB, v7 active code и v6 editable source. Тесты `test_recovery_binds_registry_active_database_not_legacy`, `test_recovery_refuses_foreign_owner_or_changed_fence` краснеют при выборе старого volume по имени/default.
- [ ] 2: orchestrator `uv run --frozen pytest -q tests/test_restoration_recovery.py`; initial red missing recovery interface. Add `test_inspection_never_starts_postgres_or_writes_original_volume`: recorder asserts no start/init/chmod/write commands to original before approved phase.
- [ ] 3: реализовать RC0/RC1 inspection: exact registry bindings/labels, current source snapshot hash, resource incarnation, snapshot inventory, DB volume path/name из registry. Не печатать inspect Env или DSN. Исполняемые names валидировать allowlist; prefix не является identity proof.
- [ ] 4: implement RC2 clone witness: source volume mount read-only, no-network pinned helper copies only confirmed synthetic DB into fresh disposable volume with run ownership. Same PostgreSQL major/image starts only clone; SQL read-only connection compares baseline row/history canonical digests and note contract. `test_clone_wal_recovery_does_not_modify_original` recomputes original manifest hash before/after; `test_raw_marker_hits_do_not_satisfy_sql_witness` requires actual SELECT result.
- [ ] 5: implement RC3 source sync with fresh recovery admission and compare-and-swap on expected **v6 page hash**. Restore exact trusted #7 page bytes, then verify full 46-file inventory; other unexpected change cancels write/recovery and returns `source_changed`. Do not reset repository/overwrite full workspace blindly. `test_recovery_replaces_only_known_stale_page` asserts no mutation to schema/migration/CRUD routes or DB.
- [ ] 6: implement RC4 start-current only after SQL witness and source sync digests persisted. Reuse exact retained DB volume, no volume reinitialize/replace and no replay of current #7 migrations into already-changed schema. Check stored migration journal against RC2; an incompatible startup chain is a blocking finding, not permission to delete note column.
- [ ] 7: RC5 uses fresh signed A/B sessions, A baseline read and isolated temporary CRUD/history witness, B denial. `test_stale_browser_cache_cannot_complete_recovery` requires fresh runtime incarnation/HTTP/probe ID; `test_start_current_never_replaces_retained_database` asserts volume unchanged. RC6 persists serving code/database/editable inventory equality and keeps #8 failed.
- [ ] 8: interruption matrix `test_recovery_restart_replays_each_gate_without_duplicate_effects` pauses after clone publication, before/after source CAS, after start before verify, after complete before response. Retry only same intent/fence; no duplicate version, no quota spend, no original DB rollback; mismatched recovery intent → 409.
- [ ] 9: green orchestrator command plus API `uv run --frozen pytest -q tests/test_project_cell_runtime_api.py tests/test_restorations.py`; run `uv run --frozen python scripts/reconcile_restoration_runtime.py --intent-file "$RECOVERY_INTENT" --phase inspect` from orchestrator, where private intent file is generated from RC0 validated actual identities. Subsequent phases execute individually after prior PASS; CLI inspect is valid in addition to mutating phase enum above.
- [ ] 10: выполнить D, commit `fix(restorations): reconcile current source and retained database authority`. Live recovery only this named synthetic project; evidence RC0–RC7 includes exact volume names, full digests, SQL witness and fresh signed runtime proof. Until RC6 PASS all normal restore/generation/preview repair remains blocked.

**Negative/observability:** wrong owner/fence/snapshot, missing active volume, missing migration journal, failed clone SQL, source CAS conflict, lost start response, resource incarnation drift. Every phase logs safe identity digests and durable progress, not raw records/credentials. Existing hard stop is not resolved by writing this helper or green unit tests alone.
...
- [ ] 1: `test_exact_empty_restore_syncs_agent_workspace_to_applied_snapshot`: v6 page and empty DB → v5 exact restoration (#7). After controller/API completed, active machine files, saved snapshot and editable tree must all match v5 bytes; current behavior 46/46 editable v6 must fail. Include code/DB volume replacement strategy and separate workspace resource identities.
- [ ] 2: red `uv run --frozen pytest -q tests/test_restoration_workspace_lineage.py`; add `test_next_generation_baseline_is_current_restored_snapshot`, `test_ready_idle_drift_is_explicit_recovery_required`, `test_new_prepare_keeps_inventory_fence` in API test file. A changed guard that simply accepts stale inventory must fail tests.
- [ ] 3: минимально добавить durable source-sync stage to existing exact activation/reconciliation. Verify source bytes before marking combined restoration readiness complete; если switch уже admitted, sync failure остаётся recoverable forward state, а не old code rollback or false completed. Binary files, modes and full 46-file inventory preserved.
- [ ] 4: in ready/idle `_prepare_source_environment`, explicit source/registry binding observation gates successor admission; no source writes on GET. If stale, return safe recovery-required with operation reference; background/operator reconciliation uses fresh ownership T20. Do not pretend return False means source consistency.
- [ ] 5: crash tests `test_crash_after_machine_switch_replays_source_sync`, `test_duplicate_source_sync_keeps_one_snapshot`, `test_changed_workspace_refuses_sync_without_overwrite`, `test_same_commit_wrong_volume_is_rejected`. Assert no DB replacement beyond existing empty strategy, no duplicate history/settlement, no generation starts before verified current baseline.
- [ ] 6: API `uv run --frozen pytest -q tests/test_restoration_source_admission.py tests/test_restorations.py tests/test_generation_flow_contract.py`; orchestrator `uv run --frozen pytest -q tests/test_restoration_workspace_lineage.py tests/test_code_restoration_engine.py`. Real Docker rehearsal compares both source surfaces, not only applied snapshot byte comparison.
- [ ] 7: выполнить D, commit `fix(restorations): synchronize editable source after exact activation`. Acceptance: repeat isolated v6→v5 exact then successor read-only baseline export matches #7/v5 before any model step. Existing LIVE-16 historical PASS remains; new assertion closes its previously uncovered workspace boundary.

**Negative/observability:** preserve source-inventory fail-closed behavior at engine lines 1176–1191. No auto acceptance of source drift; emit target/observed inventory digests and sync checkpoint. No claims that sleeping or reloading invokes reconciliation on existing code.
...
- [ ] 1: reproduce `test_runtime_probe_deadline_preserves_original_timeout`: final_build succeeds, runtime_probe crosses global ordinary-generation deadline, activity becomes timed_out, cancellation cleanup follows. Assert original stage/phase/run/lease reason survives and UI does not replace it with `cannot finish as cancelled`.
- [ ] 2: reproduce `test_terminal_cell_error_schedules_source_recovery_before_reraise`: source differs from current snapshot, terminal-cell guard throws; recovery intent must be durable even though in-handle fallback cannot run. Existing regression `test_real_terminal_cell_failure_stops_native_and_waits_release_without_snapshot` remains valid: no more model/tool/publication calls on the terminal handle.
- [ ] 3: red `uv run --frozen pytest -q tests/test_generation_terminal_recovery.py tests/test_project_cell_activity.py`; minimal change separates primary exception persistence, release completion and later source reconciliation. Do not swallow foreign lease/fence conflicts and do not remove `raise_if_terminal_cell_error` to force writes after expiry.
- [ ] 4: `test_cancel_after_timeout_is_observation_of_same_terminal_lease`, `test_foreign_terminal_lease_is_not_silently_accepted`, `test_recovery_uses_current_snapshot_not_stale_generation_baseline`, `test_failed_recovery_does_not_hide_primary_deadline`: exact identity checks, single terminal event, current #7 authoritative, secondary recovery error retained separately.
- [ ] 5: normal generation retains documented edit deadline semantics; stage/phase diagnostic distinguishes final_build/runtime_probe from model editing. If changing stage budgets is needed, measure reserved finalization budget and test it explicitly; не увеличивать лимит вслепую и не считать 1.487M input tokens доказательством performance improvement. Adaptive stage budgets already delivered remain tested.
- [ ] 6: green `uv run --frozen pytest -q tests/test_generation_terminal_recovery.py tests/test_generation_deadline.py tests/test_project_cell_activity.py tests/test_generation_flow_contract.py`; deterministic clock and real DB lease transitions, not 25-minute test sleeps. After failure no current version promotion, no duplicate terminal consumption, recovery remains visible until source/runtime verified.
- [ ] 7: выполнить D, commit `fix(generation): preserve timeout outcome and reconcile source after release`. Live acceptance on synthetic project: inject bounded deadline after build, inspect one terminal reason and durable source-sync recovery, then prove fresh signed current runtime; do not bill a new generation to recover the old one.

**Negative/observability:** `timed_out→cancelled`, simultaneous user cancel/deadline, controller response lost, cleanup release failure, source sync failure and duplicate notifications. UI preserves original error, safe recovery state and exact restoration/generation/lease IDs; no transcript/private provider payload in error.
...
- [ ] 1: `test_ready_resources_stopped_runtime_is_not_serving_proof`: workspace ready, all activity counters zero, bound DB detached, fresh preview session returns controlled unavailable/recovery status and never selects legacy DB. Current status 409 alone is recorded; test pins actionable recovery reason and absence of unsafe auto-wake.
- [ ] 2: `test_cached_preview_cannot_clear_recovery_state`, `test_preview_retries_do_not_start_unbound_database` in existing web/API suites. Expected red: stale visual evidence or generic retry loop interpreted as ready; use fresh session/status response and binding digest rather than browser cache.
- [ ] 3: minimal UI displays current saved version and runtime recovery requirement separately; retains visible failure reason and does not auto-trigger start/repair while source authority mismatch is confirmed. Ready-resource semantics need not be renamed globally; only serving proof requires actual current app/DB pair.
- [ ] 4: new `test_failed_run_cost_does_not_imply_settlement`, `test_failed_run_charge_evidence_detects_duplicate_logical_receipt` bind assistant message `d295484f-b100-4456-a576-c9c1ad343b1b`-shaped fixture to QA account and provider usage entries. Multiple legitimate model/tool charges remain allowed; duplicate logical charge and repeated terminal quota settlement fail. Missing receipt returns NOT_VERIFIED, never zero charge or PASS.
- [ ] 5: API `uv run --frozen pytest -q tests/test_restoration_runtime_authority.py tests/test_failed_generation_charge_evidence.py tests/test_project_cell_runtime_api.py`; web `pnpm test -- src/lib/__tests__/max-live-preview-recovery.test.tsx`; typecheck. Read-only live ledger inspection scopes only this synthetic failed run, does not touch balances.
- [ ] 6: выполнить D, commit `fix(runtime): expose restoration authority recovery before preview`. Evidence: stopped-runtime state and safe UI, then RC5 fresh signed preview after controlled recovery; separately ledger receipt status, amounts/currencies and no inferred refund/exactly-once claim.

**Negative/observability:** missing app container, missing DB container, stale registry, foreign active volume, absent receipt, stale tab and retired signed session. Aggregate idle + `ready` remains insufficient to resume acceptance. Hard stop is lifted by RC6 proof, not by this UI change alone.
...
10. После failed run `48e60d8e-9388-47cd-80b2-3af25b61f655` действует source/runtime/DB hard stop независимо от idle counters. Не начинать v7→v5 restore, обычный runtime restart, preview-session repair или adaptive. Запрещено выбирать legacy DB по отсутствию текущего container. Только RC0–RC7 разрешает контролируемое восстановление и последующий admission.
11. PostgreSQL startup/WAL recovery может записывать файлы даже при read-only SQL. Поэтому original frozen synthetic DB только mount-readonly; exact post-stop SELECT — на отдельном consistent clone того же major/image. Raw string counts и file hashes не заменяют SQL witness; unchanged pre-stop digests не подтверждают post-stop сохранность автоматически.
...
- `recovery-authority.json`: RC0/RC1 exact source snapshot, editable/active code inventories, active DB registry binding, full volume names and before/after fencing; отсутствие работающего app/DB фиксируется явно.
- `recovery-sql-witness.json`: RC2 clone identity, PostgreSQL image/major, original read-only source manifest before/after, exact synthetic row/history/schema assertions; тип каждого digest и canonical query version.
- `recovery-progress.json`: RC3–RC6 durable intent, source CAS, start-current without DB replacement/replay, fresh signed A/B and temporary CRUD evidence. Stale cached UI cannot satisfy any required check.
- `failed-run-outcome.json`: run/assistant/source message/lease/version IDs, original deadline and secondary cleanup status, usage/cost fields, separately observed wallet receipt state; missing receipt stays NOT_VERIFIED.
...
Критический путь после failed-v8 evidence: `hard stop → RC0/RC1 authority binding → RC2 independent SQL → RC3 source reconciliation → RC4/RC5 current-v7 serving proof → RC6 admission gate → compatible successor/restore → explicit adaptive acceptance`. Предотвращение повторения: `T21 workspace lineage + T22 terminal recovery + T19 schema-effect fencing + T23 serving authority → exact release CI/deploy/T02 → T03–T11 remaining release matrix`. T01/T12/T14 и независимые test branches могут готовиться параллельно; никакой elapsed-time wait не заменяет RC3 source reconciliation. Для T17–T19 оценка 18–30 focused блоков плюс real-DB reproduction; T20–T23 добавляют 24–40 блоков плюс cloned-DB/restart trials, без календарного обещания. Ни один mandatory gate не пропускается; изменение release инвалидирует bindings затронутых checks, а прежние historical PASS сохраняются как отдельные факты.
