...
> **Статус: история, не обязательно к чтению при старте.** Документ описывает сетап автономных
> рутин EXECUTOR/REFACTOR (июнь 2026). Их протокол расходится с действующим регламентом
> [`AGENTS.md`](../AGENTS.md): прямой push в `main`, деплой на production из каждого тика без
> разрешённого выпуска, `bypassPermissions` «не переспрашивай», обязательные личные плагины и
> SSH-алиас. Не создавать и не возобновлять эти scheduled-задачи в текущем виде без отдельного
> решения владельца, согласованного с `AGENTS.md`.
>
> **Сетап для нового участника** — не этот файл, а [`README.md`](../README.md): там перечислены
> обязательные инструменты проекта, необязательные помощники и локальные настройки пользователя.
> Базовый цикл (правка → проверки → PR) не требует плагинов, SSH-доступа к серверу и личной памяти.
...
## 1. ПРЕРЕКВИЗИТЫ (один раз; историческая версия)

> Классификация по действующим правилам: п. 2–3 (клон, собственный доступ к GitHub) — обязательны для
> любого участника; п. 4 (SSH-алиас к серверу) — локальная настройка и доступ только для разрешённых
> операций на production; п. 1, 5, 7 (scheduled tasks, Mobbin, плагины/скиллы/MCP) — необязательные
> помощники; п. 6 (`bypassPermissions`) — локальная настройка пользователя, не требование проекта.
> Пути вида `<ROMA_REPO>` и `C:\projects\...` — примеры.
