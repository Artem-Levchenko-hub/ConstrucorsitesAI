...
```bash
sudo cp /opt/omnia/infra/systemd/yleum-orchestrator.service /etc/systemd/system/
...
```bash
sudo systemctl status yleum-orchestrator
curl -sS http://127.0.0.1:8003/health
journalctl -u yleum-orchestrator -n 50 --no-pager
```
...
sudo systemctl restart yleum-orchestrator
```
...
- Sudoers rules for `nginx -t`, `systemctl reload nginx`, and `acme.sh` live
  in `/etc/sudoers.d/yleum-orchestrator` (see `docs/08-vps-setup.md`).
