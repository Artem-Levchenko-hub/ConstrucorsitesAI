...
## Состояние сквозной проверки на 22.09, вечер

`40-smoke.sh` доводит сценарий до финализации: ячейка поднимается, агент собирает приложение,
сертификат превью выпускается, публикация ждёт живой nginx (239095b0). Контрольный прогон после
починки гонки не завершён: llmgw.ru вернул 429 на тяжёлые вызовы, а затем **заблокировал ключ**
(«Key is blocked. Update via /key/unblock») — тот же ключ стоит и на старом сервере, генерации
невозможны нигде до разблокировки владельцем. После разблокировки: `./40-smoke.sh https://yleum.ru 25`
и удалить смок-проект (`DELETE /api/projects/<id>`; сейчас падает 500 из-за FK на
`project_cell_proofs` — отдельная задача).
