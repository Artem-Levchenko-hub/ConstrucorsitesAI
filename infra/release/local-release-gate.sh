...
run_step api_mypy api.log "API typecheck" bash -c 'cd apps/api && uv run mypy src'
run_step api_migrate api.log "API local database migration" bash -c \
  'cd apps/api && uv run alembic upgrade head'
run_step api_tests api.log "API tests" bash -c 'cd apps/api && uv run pytest -q'
...
run_step orchestrator_ruff orchestrator.log "Orchestrator lint" bash -c \
