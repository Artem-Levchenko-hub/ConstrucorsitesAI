...
- [~] авто-подбор стека из потребности → триаж/оркестратор — discovery РЕКОМЕНДУЕТ стек (static/fullstack/nextjs_entities) и вшивает в бриф; РОУТИНГ в провижн контейнера ещё НЕ подключён (известный V2-gap `omnia_v2_runtime_live`) — следующий шаг
- [ ] полная сборка аппа из дискавери + провижн (static-build из дискавери РАБОТАЕТ E2E; контейнер-авто-провижн — со стек-роутингом выше)
- [ ] удаление проекта юзером (кнопка+подтверждение → каскад + teardown провижена + owner-scoping 403 + бэкап/soft-delete, идемпотентно)
- [~] E2E-приёмка P1: discovery→build→preview ЗЕЛЁНАЯ вживую на проде (кофейня); остаются стек-роутинг контейнера + удаление проекта
