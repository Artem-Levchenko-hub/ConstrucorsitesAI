...
### P2 — ошибки в чате (ядро DONE @ede1c57; app-5xx DONE @e15387b; JS-console — остаток)
...
- [ ] (остаток-2) сбор **JS-console превью** (клиентские ошибки) → карта + НАТУРАЛЬНЫЙ build-error E2E (модель сама пишет битый код→карта). JS-console = отдельный заход: трогает shared preview-инжект `omnia-inspector.js` (×2-3 копии + drift-тест) + `PreviewFrame.tsx`/`StreamingPreviewFrame.tsx` + новый api-эндпоинт; РИСК ложно-позитивных карт на живых здоровых аппах (benign console-noise/404) — нужен строгий gating (только uncaught error/unhandledrejection, dedup, флаг, НЕ на публичном `/p/`).
