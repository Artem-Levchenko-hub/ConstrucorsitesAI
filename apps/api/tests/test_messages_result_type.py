"""Tests for the first-build result-type wiring in generation/onboarding (RT-1).

The router itself (classify/resolve/infer + the type→stack map) is unit-tested in
test_discovery.py; here we lock the onboarding helper ``_maybe_result_type_
...
async def test_clarify_question_off_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(onboarding, "get_settings", lambda: _settings(router=False, clarify=False))
    assert await onboarding._maybe_result_type_question("сделай сайт", "ru") is None
...
async def test_clarify_question_off_when_only_router_on(
    ...
    monkeypatch.setattr(onboarding, "get_settings", lambda: _settings(router=True, clarify=False))
    assert await onboarding._maybe_result_type_question("сделай сайт", "ru") is None
...
async def test_clarify_question_skipped_when_keyword_net_sure(
    ...
    monkeypatch.setattr(onboarding, "get_settings", lambda: _settings(router=True, clarify=True))
    ...
    monkeypatch.setattr(onboarding, "classify_result_type", _boom)
    assert await onboarding._maybe_result_type_question("запись на приём", "ru") is None
...
async def test_clarify_question_skipped_when_classifier_confident(
    ...
    monkeypatch.setattr(onboarding, "classify_result_type", _conf)
    assert await onboarding._maybe_result_type_question("сделай сайт", "ru") is None
...
async def test_clarify_question_fires_on_genuine_ambiguity(
    ...
    monkeypatch.setattr(onboarding, "classify_result_type", _unsure)
    res = await onboarding._maybe_result_type_question("сделай сайт", "ru")
