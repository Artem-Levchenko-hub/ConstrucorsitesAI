...
async def test_prepare_max_runtime_context_selects_project_cell_once_without_legacy_runtime(
    ...
    result = await runtime._prepare_max_runtime_context(
        ...
        vision_context="ctx",
        docs_media_execute=lambda _action: pytest.fail("сборка обязана идти через ячейку"),
...
async def test_execute_max_agent_action_routes_selected_runtime_check_without_legacy_bootstrap(
    ...
    result = await runtime._execute_max_agent_action(
        ...
        project_id=uuid4(),
        project_slug="max-cell",
        base_agent_executor=lambda _action: pytest.fail("сборка обязана идти через ячейку"),
