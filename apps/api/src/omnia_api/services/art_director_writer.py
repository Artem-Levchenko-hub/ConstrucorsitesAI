...
KIT: <назови вариант ГОТОВОГО кит-снипета из блока «КИТ ПРЕМИУМ-СЕКЦИЙ ЛЕНДИНГА»
  ...
  ПЕРВЫЙ ЭКРАН — ВЫБЕРИ hero-снипет ПОД АРХЕТИП визуального стиля, который ты УЖЕ выбрал в # 2 (НЕ
  всегда один и тот же первый экран): продукт/SaaS/бизнес/услуга → hero-centered (дефолт) или
  hero-split (есть сильный продукт-кадр); BOLD STUDIO / KINETIC TYPE / агентство / портфолио /
  дизайн-студия / бренд-манифест → hero-editorial (тип-как-герой, .display-fill во весь экран без
  браузер-рамки); APPLE TECH / EDITORIAL LUXURY / тёмный премиум / продукт с атмосферным фото →
  hero-cinematic (full-bleed фото + grain + ОДИН акцент). Так каждая ниша получает СВОЙ первый
  экран, а не один и тот же. Совсем нетиповой первый экран ниши — только тогда «bespoke» (вручную
  по спеке, ГЕРОЙ-РЕЖИМ I/II); нетиповую секцию ниши — тоже «bespoke». РАМА страницы — тоже из кита: ПЕРВОЙ секцией всегда
...
АРХЕТИП ГЛАВНОГО ЭКРАНА — объяви ОДИН под нишу (он диктует композицию /dashboard; НЕ
  ...
  ② ВИТРИНА-КАТАЛОГ (портфолио/магазин/недвижимость/меню/события — «продаётся глазами»):
     ...
     числа второстепенны.
  ③ ДОСЬЕ-ФОКУС (медкарта/объект/профиль/проект — ценность = одна богатая запись):
     список-выбор + <RecordDetail> (image-hero + ключевая величина + список атрибутов).
  ④ ТРЕКЕР-ПОТОК (заявки/тикеты/заказы/воронка — запись по стадиям): <DashboardHero> со
     счётчиками-по-стадиям (BarMini-разбивка) + <CrudResource> с фильтром-по-статусу и
     цветным Badge (канбан-доску руками НЕ верстать — её нет в ките).
...
ГЛАВНЫЙ ЭКРАН "/" (по выбранному архетипу — НЕ всегда DashboardHero):
  ...
  [② КАТАЛОГ] вместо большого hero — компактная <PageHeader>-полоса (1-2 числа) +
    ДОМИНАНТА = сетка карточек с обложками; [③ ДОСЬЕ] список-выбор + <RecordDetail>
    одной записи. (Чистый личный трекер без «главной» метрики — можно без hero, тогда
...
КАЖДАЯ СУЩНОСТЬ — страница "/<route>" через <CrudResource entity="<Имя>">:
  ...
  ВИД: для ВИЗУАЛЬНОЙ сущности (товар/каталог, недвижимость, меню/блюдо, портфолио,
    событие, авто, тур — то, что «продаётся глазами») назначь `view="gallery"` +
    media-маппинг (обложка/заголовок/подзаголовок/цена/бейдж) и заведи ей поле-обложку
    image (kind:"image") — это сетка карточек с картинкой уровня Airbnb/Shopify, а НЕ
    текстовая таблица. Для деловых сущностей (клиенты, сделки, задачи, счета) — таблица.
