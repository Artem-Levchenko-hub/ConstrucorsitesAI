...
— ГЛАВНЫЙ ЭКРАН (hero) ВСЕГДА с ВИДИМЫМ фоном: тематическое фото-фон full-bleed
  (<img data-omnia-gen=...> absolute inset-0 object-cover) ИЛИ нарисованный граф-фон
  (.omnia-shader тон-в-тон / .bg-mesh / .fx-aurora-soft + орбы). ФОН ДОЛЖЕН БЫТЬ
  ВИДЕН: затемнение поверх держи УМЕРЕННЫМ — макс ~`/45-/55`, НИКОГДА не `/70`…`/90`
  и не сплошная чёрная заливка (иначе фон не виден, экран читается как «плоский
  монотонный»). Текст защищай ЛОКАЛЬНО (text-shadow, мягкий градиент только снизу
  под текстом), а НЕ глобальной чернотой на весь экран.
