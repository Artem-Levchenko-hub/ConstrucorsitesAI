...
— ИНТРО→СТРАНИЦА (усиливает первое впечатление, если уместно вайбу): короткий прелоадер-оверлей через класс `.omnia-intro` (fixed, бренд-вордмарк/лого по центру, опц. рисованный SVG) — КИТ САМ плавно растворяет его в hero (CSS-авто-fade ~1.6s, reduced-motion прячет). Один на страницу, первым в body. НЕ пиши свой fade-JS — только класс `.omnia-intro`.
