...
KIT: <назови вариант ГОТОВОГО кит-снипета из блока «КИТ ПРЕМИУМ-СЕКЦИЙ ЛЕНДИНГА»
  системного промпта — hero-centered | hero-split | features-grid | pricing-plans |
  testimonial-wall | faq-accordion | cta-band — верстальщик соберёт секцию из снипета 1:1
  (тот же дизайн-ДНК, что у entity-аппов). ПЕРВЫЙ ЭКРАН продукта/SaaS/бизнеса/услуги —
  hero-centered (дефолт) ИЛИ hero-split (есть сильный продукт-кадр). Только редакционный/
  агентский/портфолио-hero пиши «bespoke» (вручную по спеке); нетиповую секцию ниши — тоже
  «bespoke». Лендинг ОБЯЗАН закрываться cta-band перед футером.>
...
• КИТ-СЕКЦИИ: если у секции в брифе поле `KIT: <hero-centered|hero-split|features-grid|
  pricing-plans|testimonial-wall|faq-accordion|cta-band>` — БЕРИ соответствующий готовый HTML-снипет
...
1. Все секции брифа на месте, в том же порядке, с теми же id? Каждая секция с полем
   ...
   Tailwind), палитра-якорь `:root{--primary…--line}` задана в <style>, лендинг закрыт cta-band?
