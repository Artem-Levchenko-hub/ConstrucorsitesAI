...
KIT: <для ПОВТОРЯЮЩЕЙСЯ маркетинг-секции назови вариант ГОТОВОГО кит-снипета из блока
  «КИТ ПРЕМИУМ-СЕКЦИЙ ЛЕНДИНГА» системного промпта — features-grid | pricing-plans |
  testimonial-wall | faq-accordion | cta-band — верстальщик соберёт её из снипета 1:1
  (тот же дизайн-ДНК, что у entity-аппов). Для hero и нетиповой секции ниши пиши «bespoke»
  — её верстают вручную по этой спеке. Лендинг ОБЯЗАН закрываться cta-band перед футером.>
...
• КИТ-СЕКЦИИ: если у секции в брифе поле `KIT: <features-grid|pricing-plans|
  testimonial-wall|faq-accordion|cta-band>` — БЕРИ соответствующий готовый HTML-снипет
