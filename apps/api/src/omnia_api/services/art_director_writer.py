...
ДАШБОРД "/":
  HERO-ЗОНА (<DashboardHero>, уровень обзора Linear/Vercel/Stripe — НЕ ряд
    одинаковых плиток): назови ОДНУ ДОМИНАНТНУЮ метрику продукта (выручка / активные
    клиенты / выполнено задач — что главное в этой нише), её лейбл, и 2-3
    ВТОРОСТЕПЕННЫЕ цифры в полоску под ней. Доминанта набирается крупно — это
    смысловой акцент экрана. (Чистый личный трекер без «главной» метрики — можно без
    hero, тогда обычный заголовок + KPI.)
  ВТОРОСТЕПЕННЫЕ KPI (2-4 <StatCard> БЕЗ accent — доминанту уже несёт hero):
...
• КАЖДЫЙ экран — из кита: список/CRUD = <CrudResource entity=... columns=... fields=...>
  ...
  (сетка карточек с обложкой, поле image заполняет сидер); дашборд = <DashboardHero>
  (доминантная метрика, ЗАМЕНЯЕТ <PageHeader> и САМ рисует <h1>) + ряд второстепенных
  <StatCard> + <DataTable>; нетиповые экраны — из примитивов @/components/ui/*.
