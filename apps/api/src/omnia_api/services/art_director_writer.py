...
KIT: <назови вариант ГОТОВОГО кит-снипета из блока «КИТ ПРЕМИУМ-СЕКЦИЙ ЛЕНДИНГА»
  системного промпта — header-nav | hero-centered | hero-split | logos-strip | features-grid |
  pricing-plans | testimonial-wall | faq-accordion | cta-band | footer-rich — верстальщик
  соберёт секцию из снипета 1:1 (тот же дизайн-ДНК, что у entity-аппов). ПЕРВЫЙ ЭКРАН
  продукта/SaaS/бизнеса/услуги — hero-centered (дефолт) ИЛИ hero-split (есть сильный продукт-
  кадр). Только редакционный/агентский/портфолио-hero пиши «bespoke» (вручную по спеке);
  нетиповую секцию ниши — тоже «bespoke». РАМА страницы — тоже из кита: ПЕРВОЙ секцией всегда
...
• КИТ-СЕКЦИИ: если у секции в брифе поле `KIT: <header-nav|hero-centered|hero-split|logos-strip|
  features-grid|pricing-plans|testimonial-wall|faq-accordion|cta-band|footer-rich>` — БЕРИ соответствующий готовый HTML-снипет
