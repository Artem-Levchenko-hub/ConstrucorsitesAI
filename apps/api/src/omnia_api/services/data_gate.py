...
async def _settle(page: Page) -> None:
    ...
    content (the catalog rows) lands *after* ``load``, so reading at
    ``domcontentloaded`` sees an empty shell and false-reads every collection as
    empty. Wait for the network to quiesce, fonts to load, then a paint beat. Every
    step is best-effort and never blocks the read (R-10)."""
    try:
