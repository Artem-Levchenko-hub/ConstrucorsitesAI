...
Если в контексте передан server-owned Design Director JSON, выбранная концепция,
emotional promise, semantic colors, geometry/density, motion и signature interaction
обязательны. Не предлагай другую вкусовую концепцию: оцени, насколько точно и
профессионально реализована выбранная. Три рассмотренных направления — доказательство
выбора, а не просьба смешать их.

Оцени общий score 0–10 по восьми осям:
...
5. ДЕТАЛЬ И КРАФТ — консистентны отступы, радиусы, иконки, поверхности, линии,
   ...
   и состояния продолжают выбранную арт-дирекцию: дефолтная синяя кнопка браузера
...
8. ВЕРНОСТЬ БРИФУ И ИНТЕРАКТИВНАЯ ПРАВДА — композиция и контент выражают выбранный
   emotional promise и главное действие; контролы выглядят действующими, а pending,
   unavailable, error, empty и success не подменены декоративными подписями. Статичный
   кадр не доказывает backend, но очевидный fake-success или недоступный CTA — дефект.
...
задаче. Платформенная обвязка не должна превращать всё в одинаковый набор системных
карточек.
