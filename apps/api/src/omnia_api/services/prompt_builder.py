...
hero-split). РАМА страницы — тоже из кита: header-nav (липкая шапка) ПЕРВЫМ элементом
...
секции бриф называет KIT-вариант (header-nav | hero-centered | hero-split | logos-strip |
features-grid | pricing-plans | testimonial-wall | faq-accordion | cta-band | footer-rich) —
...
▸ АРХЕТИП ГЛАВНОГО ЭКРАНА — ВЫБЕРИ ОДИН ПОД НИШУ ДО ВЁРСТКИ. Это решает композицию
  ...
  ③ ДОСЬЕ-ФОКУС — одна богатая запись в центре. Ниши: медкарта пациента, объект
     недвижимости, профиль/резюме, проект, дело/кейс, бронь, договор — где единица
     ценности = ОДНА запись, а не поток. Главный экран: компактный список-выбор (слева
     или сверху) → <RecordDetail> выбранной записи (image-hero + крупная ключевая
     величина + список атрибутов). Доминанта = САМА запись, не сетка чисел. Плотность
     НИЗКАЯ-СРЕДНЯЯ.
  ④ ТРЕКЕР-ПОТОК — запись движется по стадиям. Ниши: заявки, тикеты, заказы, лиды-
     ...
     СЧЁТЧИКАМИ-ПО-СТАДИЯМ (chart={<BarMini data={разбивка по статусу} />}) →
     <CrudResource>/<DataTable> с фильтром-сегментом по статусу сверху + статус каждой
     строки цветным <Badge>. ⛔ Канбан-доски в ките НЕТ — НЕ верстай колонки-стадии
     руками сырым Tailwind (выйдет криво); стадии несёт фильтр + бейдж + BarMini-разбивка.
     Плотность ВЫСОКАЯ.
  ▸ Сомнение между двумя — бери тот, чья ГЛАВНАЯ ЗАДАЧА юзера ближе (смотреть числа →
    ①; листать визуально → ②; изучать одну запись → ③; двигать по воронке → ④). Гибрид
    допустим (каталог + KPI-полоса), но ДОМИНАНТА на экране — РОВНО ОДНА.
