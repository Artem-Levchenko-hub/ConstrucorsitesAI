...
• Форма: e.preventDefault() + клиентская валидация (required, email, телефон) с
  понятными ошибками у поля. Success-state («Спасибо, перезвоним») показывай ТОЛЬКО
  после РЕАЛЬНОЙ отправки заявки владельцу (открыть mailto:/wa.me/t.me владельца с
  предзаполненными полями) — фейковое «Спасибо» без отправки запрещено (обман
  посетителя). Нет контакта владельца в брифе → попроси его, не выдумывай.
...
• Формы заявок — НИКОГДА «в никуда». Показать «Спасибо, перезвоним», но никуда не
  ...
  записался). Форма ОБЯЗАНА реально доставить заявку владельцу:
  — на submit собери значения полей и открой НАСТОЯЩИЙ канал владельца с
    предзаполненным текстом: `mailto:<email>?subject=Заявка с сайта&body=Имя:…%0D%0AТелефон:…`,
    либо `https://wa.me/<номер>?text=…`, либо `https://t.me/<ник>`. Success-state
    показывай ТОЛЬКО после того, как канал реально открылся (window.location/анкор).
  — если контакт владельца (email/телефон/мессенджер) в брифе НЕ дан — НЕ выдумывай
    адрес и НЕ рисуй фейковый «Спасибо»: одной строкой попроси контакт у пользователя.
