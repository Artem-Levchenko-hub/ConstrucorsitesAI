...
▸ АРХИТЕКТУРА — ДВА СЛОЯ: ПУБЛИЧНАЯ ВИТРИНА + ЗАКРЫТЫЙ КАБИНЕТ (НЕ один экран).
  ...
  ▸ ВИТРИНА / КАТАЛОГ НА ГЛАВНОЙ (ниша магазин/маркетплейс/афиша — главная
    показывает СПИСОК товаров из публичной сущности, `entities.X.list()` в useEffect).
    Три ЖЁСТКИХ правила, иначе брак:
    • КАРТИНКА КАЖДОГО ТОВАРА — из ЕГО поля: `<img src={item.imageUrl || PLACEHOLDER}>`.
      ⛔ НИКОГДА не хардкодь ОДИН src на все карточки (иначе весь каталог = одна
      картинка). Нет imageUrl → нейтральный плейсхолдер (CSS-градиент или один
      дефолт-проп), но `alt`/title всегда из записи.
...
▸ ДВЕ РОЛИ — ОПЕРАТОР (admin) vs КЛИЕНТ (user). Решай В НАЧАЛЕ: есть ли у этого
  ...
  • ДВА типа (клиника: персонал ведёт пациентов + пациенты записываются; магазин:
    ...
    ▸ ОПЕРАТОР бизнеса (владелец, кто сгенерил апп) — первый зарегавшийся юзер
      ...
      Эти сущности → `"access": "admin"` (движок 403-ит не-админа). CRUD-страницы
      персонала держи в роль-гейтнутом разделе `(app)/admin/...`, и КАЖДУЮ ссылку на
      них в навигации оборачивай `<Protected role="admin" behavior="hide">` + серверно
      гейть `requireUser({ role: "admin" })`. Layout раздела `/admin` в начале гонит
      не-админа прочь: `if (me && me.role !== "admin") router.replace("/dashboard")`.
    ...
    ⚠️ ВХОД (рег/логин) ВСЕГДА ведёт в `/dashboard` (APP_HOME фиксирован, не меняй) →
      ...
      для клиента (role=user): самообслуживание, НЕ пустая чужая админка. Nav `/dashboard`
      role-aware — операционные ссылки только под `<Protected role="admin">`, клиент
      видит лишь своё. НЕ делай отдельный клиентский шелл (`/client`, `/account`) как
      его дом: после входа клиент попадёт в `/dashboard`, а не туда, и застрянет в
      чужом разделе с пустыми admin-списками (403 «admin only») = тупик.
    ⛔ НИКОГДА не показывай клиенту управление операционными данными бизнеса (список
      ВСЕХ пациентов/заказов/сотрудников). Гейть `requireUser({ role: "admin" })` на
      серверных admin-страницах + `<Protected role="admin">` на UI + `access:"admin"`
      на сущности — все три слоя.
