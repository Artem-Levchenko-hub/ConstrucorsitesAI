...
_IMAGE_GEN_ON = """\
ФОТО НА САЙТЕ — ДВА ИСТОЧНИКА, используй ОБА по назначению:

① data-omnia-gen → ГЕНЕРАЦИЯ (платно) — ТОЛЬКО ОДИН кадр: ФОН ГЛАВНОГО ЭКРАНА (hero).
   Один уникальный концепт-кадр под бренд, больше ничего сюда не клади.
<img data-omnia-gen="english prompt: subject, scene, style, lighting, angle, lens"
   ...
   • РОВНО 1 такой тег на странице (hero-фон). Лишние Omnia обрежет — кошелёк зря.
   • Промпт на английском, конкретный: «cinematic wide shot, artisan coffee bar
     interior at golden hour, warm bokeh, 35mm, shallow depth of field» — НЕ «кафе».
   • НЕ трать gen на товары/еду/портреты/команду/услуги/галерею — это идёт в ②.

② data-omnia-photo → СТОК-ФОТО (БЕСПЛАТНО) — СЦЕНЫ И ОБЪЕКТЫ, НЕ ЛЮДИ:
   товары/продукты, услуга в процессе, интерьер, оборудование/инструменты,
   галерея, блюда, атмосфера, фоны не-hero секций. ЛЮДЕЙ/ПОРТРЕТЫ сюда НЕ клади (③).
<img data-omnia-photo="english keywords: subject, setting, mood"
     alt="русский alt"
     style="background:linear-gradient(135deg,var(--brand,#1e293b),var(--accent,#0ea5e9));"
...
• ВСЕГДА inline-градиент в style= под палитру (var(--brand)/var(--accent)) — виден
  ДО подмены (вместо broken-image иконки).
• Фиксируй размеры классами (w-full h-72/h-96/h-full) — иначе layout-сдвиг.
• Тема тега совпадает с alt и контекстом секции.
• Для fullstack — обычный <img>, НЕ next/image (resolver работает на raw HTML).
• 💸 ПОВТОРЯЮЩИЕСЯ КАРТОЧКИ: для gen-сеток дай ВСЕМ общий
  `data-omnia-gen-group="cards"` (одна генерация на группу). Для photo-сеток просто
  повтори ОДНИ И ТЕ ЖЕ keywords — Omnia скачает одно фото на уникальный запрос и
  переиспользует. Разнообразие — разными alt/подписями/ценами, не дорогими кадрами.

③ ЛЮДИ / ПОРТРЕТЫ (команда, врачи, мастера, специалисты, аватары отзывов) → НЕ фото,
   а ЧИСТЫЙ АВАТАР-ИНИЦИАЛЫ: круг с инициалами в фирменном цвете (CSS, без картинки).
   Бесплатный сток беден на людей и даёт мимо-кадры (чужие/одинаковые лица) — поэтому
   людей рисуем инициалами. НИКОГДА не ставь data-omnia-photo/gen на портрет человека.
...
кафе, клиника): 1 hero-фон data-omnia-gen + 3–6 data-omnia-photo (товары, услуги,
интерьер, галерея, атмосфера); команда/специалисты — аватарами-инициалами (③). Сайт
такого типа без единого реального фото-сцены = провал.
...
data-omnia-gen / data-omnia-photo или инлайн-SVG-иллюстрации."""
