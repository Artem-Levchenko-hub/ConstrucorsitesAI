...
_ART_DIRECTOR = """\
АРТ-ДИРЕКЦИЯ — ГЛАВНОЕ, читай первым. ПЕРЕД версткой мысленно придумай ОДНОРАЗОВЫЙ
дизайн-концепт именно под ЭТОТ бриф (не выбирай из меню — ИЗОБРЕТАЙ):
• характер одним словом — что человек чувствует за 1 секунду;
• light или dark; ОДИН доминирующий тон + ОДИН приглушённый акцент + нейтраль.
  НЕ три ярких цвета вперемешку — это выглядит дёшево;
• пара характерных шрифтов (выразительный дисплейный + чистый текстовый),
  НЕ Space Grotesk и НЕ Inter по умолчанию;
• одна сигнатурная идея движения и один структурный wow-приём (нестандартная
  раскладка, НЕ банальный «текст слева / картинка справа»);
• как выглядит первый экран, чтобы зацепить с первого взгляда.

ГЛУБИНА, НЕ ПЛОСКО (плоское = дешёвое, это брак):
• НИКАКИХ плоских заливок чистым ярким цветом во всю карточку/секцию. Поверхности —
  градиент тон-в-тон, ИЛИ живой шейдер, ИЛИ многослойная тень с цветным подтоном
  (.shadow-tinted / .depth-2 / .depth-3) + фактура (.grain).
• Кнопки ОБЪЁМНЫЕ: градиент + тень в цвет бренда + лёгкий внутренний хайлайт (inset
  highlight). Плоская одноцветная пилюля = брак. Кнопки сторов — НАСТОЯЩИЕ бейджи
  App Store / Google Play (тёмные, логотип + две строки), НЕ текстовые пилюли.
• Яркий цвет — ДОЗОЙ (акцент, один CTA, highlight), НЕ как фон целых блоков.
  Сложные/приглушённые оттенки выглядят дорого; чистые насыщенные рядом — дёшево.

ПЛАНКА КРАФТА — уверенность и цельность. Ориентир характеров (НЕ копируй —
подбирай по смыслу бизнеса): кинематографичный editorial (тёмный, антиква, латунь);
дерзкий brutalist (хард-эджи, гигантский тип, контраст); премиум-playful (тепло,
глянец, ГЛУБИНА — но БЕЗ плоских кислотных заливок). Два сайта подряд — РАЗНЫЕ.
"""
