...
• 💸 ОДНА КАРТИНКА НА ПОВТОРЯЮЩИЕСЯ КАРТОЧКИ (экономия). Сетки одинаковых карточек
  (меню, товары, портфолио-плитки, плейсхолдер-аватары) НЕ генери по отдельному
  фото на каждую — это дорого и не нужно. Дай ВСЕМ карточкам секции общий атрибут
  `data-omnia-gen-group="cards"` (имя группы — своё на секцию: cards / menu /
  team / gallery). Omnia сгенерит ОДНУ картинку на группу и подставит во все.
  Реальный промпт нужен на ПЕРВОЙ карточке группы; у остальных оставь тот же
  data-omnia-gen (он проигнорится). data-omnia-gen БЕЗ группы = уникальная
  генерация — оставляй её ТОЛЬКО для КОНЦЕПТ-кадров (hero, фирменное фото, главный
  визуал). Разнообразие карточкам дают разные alt/подписи/цены и компоновка, а не
  разные дорогие фото.

КОГДА ИСПОЛЬЗОВАТЬ:
• Hero-фото продукта/блюда/услуги (1 крупный кадр);
• Карточки портфолио / меню — ОДНА общая картинка на всю сетку (data-omnia-gen-group="cards");
• Аватары отзывов (3 портрета);
• Команда (3–6 портретов);
• Галерея интерьера (4–8 кадров).

СТОК-ФОТО (Pexels) — для ТИПОВЫХ / атмосферных кадров используй <img data-omnia-photo="english keywords">
(интерьер, природа, общие планы, фон секции, ингредиенты, города): быстрее и дешевле gpt-image,
Omnia подтянет реальное фото с Pexels. Пример:
<img data-omnia-photo="cozy japanese izakaya interior, warm ambient light"
     alt="интерьер ресторана" class="w-full h-72 object-cover rounded-2xl" />
Делёж: data-omnia-gen → hero / фирменное / специфичное (твоё блюдо, твой продукт, портрет);
data-omnia-photo → типовое / атмосферное / фон. Оба подменяются на РЕАЛЬНЫЕ картинки.

ОБЯЗАТЕЛЬНЫЙ МИНИМУМ (контентные сайты — ресторан, услуги, портфолио, отель, магазин, кафе):
≥1 hero-кадр data-omnia-gen + 3–6 data-omnia-photo (галерея / меню / атмосфера). Сайт такого
типа БЕЗ единого реального фото (только CSS-узоры) = провал, так НЕЛЬЗЯ.
...
▸ BEFORE/AFTER — перетаскиваемое сравнение `.compare` «до/после». Мгновенное «вау»
  ...
  Оба <img> — один кадр/ракурс (одинаковый размер), иначе сравнение «плывёт».
...
def _build_edit_messages(
    ...
    """
    system = "\n\n".join(
        (_EDIT_IDENTITY, _EDIT_KIT_HINT, _EDIT_BG_HINT, _EDIT_RESPONSE)
    )
...
• ФОН блока задают СЛОИ-ПОДЛОЖКИ (не контент). Чтобы СМЕНИТЬ фон ЗАМЕТНО — поменяй
  ...
  (он поверх). Картинки `<img>` не трогай — оставь те же `src`.
