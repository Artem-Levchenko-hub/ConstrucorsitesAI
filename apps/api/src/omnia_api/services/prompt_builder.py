...
Дизайн «из коробки» задаётся `_DESIGN_KIT` (палитры + пары шрифтов под индустрию) и
`_STYLE_KIT` (7 визуальных пресетов); анимации и интерактив — встроенным китом
...
_STYLE_KIT = """\
ВИЗУАЛЬНЫЙ СТИЛЬ — выбери ОДИН пресет под нишу (палитру бери из дизайн-кита выше,
пресеты её переиспользуют). Это снимает с тебя «изобретение» вида: берёшь готовую
связку приёмов и наполняешь контентом. Классы (.bg-aurora, .glass, .card-soft,
.reveal, .hover-lift, .gradient-text…) даёт кит анимаций — см. ниже.

1. Aurora SaaS (светлый) — герой на .bg-aurora (переменные --a1/--a2/--a3 под
   палитру), заголовок .gradient-text, фичи в .card-soft с .reveal+stagger и
   .hover-lift. Для: SaaS, IT, стартап, диджитал.
2. Glass Premium (тёмный) — фон .bg-mesh, шапка/карточки .glass-dark, золотой или
   светлый акцент, крупная типографика. Для: luxury/премиум, фитнес, авто.
3. Soft Clay (светлый) — claymorphism: rounded-3xl, мягкие тени (.card-soft),
   пастельный фон, дружелюбно. Для: бьюти/спа, детское, образование.
4. Editorial Serif (светлый) — serif-заголовки, много воздуха, .divider-fade вместо
   рамок, минимум цвета, спокойные .reveal. Для: блог, недвижимость, юристы, медиа.
5. Bold Brutalist (светлый) — плоские жёсткие тени, толстые бордеры, контраст,
   крупные буквы. Для: портфолио, креатив, ивенты, агентства.
6. Warm Appetite (светлый) — тёплый фон/градиент, фото-карточки .card-soft с
   .hover-lift, аппетитные акценты. Для: ресторан/кафе/доставка, ритейл.
7. Clean Trust (светлый) — минимал, спокойные сине-бирюзовые, строгая сетка, только
   деликатные .reveal. Для: медицина/клиника, B2B/консалтинг, финансы, госуслуги.

Один стиль на весь сайт — не смешивай пресеты. Тёмная тема — для tech/премиума/
гейминга: фон из палитры, текст светлый, контраст проверь отдельно."""
...
JS-хуки (просто проставь id/классы — логика уже в ките, лишний JS не нужен):
