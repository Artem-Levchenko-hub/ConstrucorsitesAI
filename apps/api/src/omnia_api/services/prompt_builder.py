...
▸ ДВЕ РОЛИ — ОПЕРАТОР (admin) vs КЛИЕНТ (user). Решай В НАЧАЛЕ: есть ли у этого
  ...
  • ДВА типа (клиника: персонал ведёт пациентов + пациенты записываются; магазин:
    ...
    ▸ ОПЕРАТОР бизнеса (владелец, кто сгенерил апп) — первый зарегавшийся юзер
      ...
      Эти сущности → `"access": "admin"` (движок 403-ит не-админа); их CRUD-страницы
      под РОЛЬ-ГЕЙТНУТЫМ разделом (напр. `(app)/admin/...`), и КАЖДАЯ ссылка на них в
      навигации обёрнута `<Protected role="admin" behavior="hide">` — клиент НИКОГДА
      не видит и не открывает управление чужими данными.
    ▸ КЛИЕНТ (обычный посетитель, кто регается на лендинге) — роль `user`. В его
      /dashboard ТОЛЬКО САМООБСЛУЖИВАНИЕ: его записи/заказы/избранное/профиль —
      сущности `owner` (видит только своё). Общий каталог для просмотра — `public`.
    ⛔ НИКОГДА не клади управление операционными данными бизнеса (список ВСЕХ
      пациентов/заказов) в клиентский /dashboard как `owner`/обычный раздел — клиент
      увидит чужую кухню и тупик. Гейть `requireUser({ role: "admin" })` на серверных
      admin-страницах + `<Protected role="admin">` на UI + `access:"admin"` на сущности.
