...
log = structlog.get_logger("yleum_orchestrator.shell")
...
async def run(
    cmd: list[str], *, timeout_seconds: float = 30.0, env: dict[str, str] | None = None
) -> CmdResult:
    """Run `cmd` (no shell), capture output, enforce `timeout_seconds` seconds.
