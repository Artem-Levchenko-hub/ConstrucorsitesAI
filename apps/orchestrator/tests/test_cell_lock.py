...
@pytest.mark.asyncio
async def test_named_hold_serializes_different_workspaces_on_host_capacity(tmp_path) -> None:
    lock = WorkspaceOperationLock(tmp_path, acquire_timeout_seconds=1, retry_interval_seconds=0.01)
    entered = asyncio.Event()
    release = asyncio.Event()
    second_entered = False
    ...
    async def first() -> None:
        async with lock.hold_named("host-capacity-admission"):
            entered.set()
            await release.wait()
...
async def test_hardlinked_lock_file_is_rejected(tmp_path) -> None:
    lock = WorkspaceOperationLock(tmp_path, acquire_timeout_seconds=1, retry_interval_seconds=0.01)
    workspace_id = uuid4()
    lock_path = tmp_path / "locks" / f"workspace-{workspace_id}.lock"
