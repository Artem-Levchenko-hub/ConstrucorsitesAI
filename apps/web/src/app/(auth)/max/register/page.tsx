...
export default async function MaxRegisterPage() {
  ...
  return (
    <main className="min-h-screen bg-[#080a10] px-5 py-8 text-white">
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        <Link href="/" className="flex items-center gap-2 text-sm text-white/55 hover:text-white">
          <ChevronLeft className="size-4" />
          На главную
        </Link>
        <Link href="/login?next=/max" className="text-sm text-[#8b5cf6] hover:text-white">
          Уже есть аккаунт
        </Link>
      </div>
      ...
      <div className="mx-auto grid max-w-5xl gap-12 py-14 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        <section>
          <div className="mb-6 flex size-11 items-center justify-center border border-white/20 bg-[#3b82f6]">
        ...
        <section className="rounded-xl border border-white/10 bg-[#0f121f] p-6 sm:p-8">
