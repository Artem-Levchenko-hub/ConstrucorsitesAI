...
function MiniAppPhone() {
  return (
    <div className="mx-auto w-[260px] rounded-[38px] border-[8px] border-[#0d0d0c] bg-[#1c1e23] p-3 shadow-[0_30px_70px_rgba(0,0,0,.22)]">
      <div className="mx-auto h-1 w-16 rounded-full bg-black/18" />
      <div className="mt-3 overflow-hidden rounded-[22px] bg-[#191b20]">
        <div className="bg-[#1c1e23] p-5 text-white">
          ...
          <div className="mt-2 h-1.5 rounded-full bg-white/15"><div className="h-full w-3/5 rounded-full bg-[#4f81f7]" /></div>
        </div>
        <div className="space-y-2 p-3">
          {["Заказать кофе", "Мои награды", "Персональные акции"].map((item, index) => (
            <div key={item} className="flex items-center gap-3 rounded-[12px] border border-[#25272b] bg-[#191b20] p-3">
              <span className={`grid size-8 place-items-center rounded-[8px] ${index === 0 ? "bg-[#4f81f7] text-[#121519]" : "bg-[#2b2d32] text-[#9fa1b1]"}`}>
              ...
              <ChevronRight className="ml-auto size-3 text-[#828491]" />
            </div>
          ))}
        </div>
      </div>
...
export default function MaxProductPage() {
  return (
    <main className="min-h-screen bg-[#121519] text-white">
      <section data-graphite-shell className="bg-[#121519]">
        ...
        <div className="mx-auto grid max-w-[1320px] gap-14 px-5 pb-24 pt-18 sm:px-8 lg:grid-cols-[1.15fr_.85fr] lg:items-center lg:pb-32 lg:pt-24">
          <div>
            <p className="omnia-kicker text-[#4f81f7]">Omnia / MAX Studio</p>
          ...
          <div className="relative min-h-[470px]">
            <div className="absolute inset-0 rounded-full bg-[#4f81f7]/10 blur-3xl" />
      ...
      <section id="about" className="mx-auto max-w-[1320px] px-5 py-24 sm:px-8 lg:py-32">
        <div className="grid gap-12 lg:grid-cols-[1fr_.8fr] lg:items-center">
          <div>
            <p className="omnia-kicker text-[#4f81f7]">MAX Mini App</p>
            ...
            <p className="mt-6 max-w-[620px] text-base leading-7 text-[#9fa1b1]">
            ...
            <div className="mt-10 grid gap-px overflow-hidden rounded-[12px] border border-[#2b2d32] bg-[#2b2d32] sm:grid-cols-2">
              {[
                ...
                return (
                  <article key={String(title)} className="bg-[#191b20] p-6">
                    <ItemIcon className="size-5 text-[#4f81f7]" />
                    <h3 className="mt-8 font-semibold">{String(title)}</h3>
                    <p className="mt-2 text-sm leading-6 text-[#9fa1b1]">{String(text)}</p>
                  </article>
                );
              })}
            </div>
      ...
      <section id="bot" data-graphite-shell className="bg-[#121519] px-5 py-24 sm:px-8 lg:py-32">
      ...
      <section className="mx-auto max-w-[1320px] px-5 py-24 sm:px-8 lg:py-32">
        <p className="omnia-kicker text-[#4f81f7]">Что делает студия</p>
        <div className="mt-4 grid gap-8 border-b border-[#2b2d32] pb-12 lg:grid-cols-2">
          ...
          <p className="max-w-[560px] text-base leading-7 text-[#9fa1b1] lg:justify-self-end">Пользователь принимает продуктовые решения и предоставляет доступы только там, где внешний сервис не имеет публичного API.</p>
        </div>
        <div className="mt-4 divide-y divide-[#2b2d32]">
          {automation.map(([Icon, title, text], index) => (
            <article key={title} className="grid gap-4 py-8 sm:grid-cols-[64px_300px_1fr]">
              <span className="grid size-10 place-items-center rounded-[8px] bg-[#2b2d32]"><Icon className="size-5 text-[#4f81f7]" /></span>
              <h3 className="text-xl font-semibold">{String(index + 1).padStart(2, "0")}. {title}</h3>
              <p className="max-w-[620px] text-sm leading-6 text-[#9fa1b1]">{text}</p>
            </article>
          ))}
        </div>
      </section>
          ...
          <div className="mt-14 grid gap-4 md:grid-cols-3">
            {[
              ...
              <article key={number} className="omnia-card p-7">
                <span className="font-mono text-[10px] text-[#4f81f7]">{number}</span>
                <h3 className="mt-10 text-xl font-semibold">{title}</h3>
                <p className="mt-3 text-sm leading-6 text-[#9fa1b1]">{text}</p>
              </article>
            ))}
          </div>
      ...
      <section id="integrations" data-graphite-shell className="bg-[#121519] px-5 py-24 sm:px-8 lg:py-32">
        <div className="mx-auto max-w-[1320px]">
          <div className="grid gap-10 lg:grid-cols-2">
            <div>
              <p className="omnia-kicker text-[#4f81f7]">Готовые подключения</p>
          ...
          <div className="mt-12 grid gap-px overflow-hidden rounded-[12px] border border-white/14 bg-white/14 sm:grid-cols-2 lg:grid-cols-4">
            {["ЮKassa / СБП", "iiko / r_keeper", "Битрикс24 / amoCRM", "МойСклад / 1С", "Yclients", "CDEK", "Яндекс Метрика", "Собственный REST API"].map((item, index) => (
              <div key={item} className="min-h-[130px] bg-[#121519] p-6">
                <div className="flex items-center justify-between"><PlugZap className="size-4 text-[#4f81f7]" /><span className="font-mono text-[9px] text-white/25">{String(index + 1).padStart(2, "0")}</span></div>
      ...
      <section id="launch" className="mx-auto max-w-[1320px] px-5 py-24 sm:px-8 lg:py-32">
        <div className="grid gap-12 lg:grid-cols-[1fr_.9fr]">
          <div>
            <p className="omnia-kicker text-[#4f81f7]">После публикации</p>
            <h2 className="mt-4 text-[40px] font-semibold leading-[1.04] tracking-[-.045em] sm:text-[56px]">Приложение остаётся управляемым</h2>
            <p className="mt-6 max-w-[600px] text-base leading-7 text-[#9fa1b1]">У каждой публикации есть версия, журнал и health-check. Новые правки проходят ту же проверяемую цепочку и не запускают генерацию повторно после обновления страницы.</p>
          </div>
          <div className="omnia-card overflow-hidden">
            <div className="flex items-center justify-between border-b border-[#2b2d32] p-5">
              <div><p className="omnia-kicker text-[#828491]">Production</p><p className="mt-1 font-semibold">coffee-miniapp.ru</p></div>
              <span className="rounded-full bg-[#248a4b]/10 px-3 py-1 text-xs font-medium text-success-fg">Работает</span>
            </div>
            <div className="grid grid-cols-3 divide-x divide-[#2b2d32]">
              {[["99.98%", "Uptime"], ["v.12", "Версия"], ["142 ms", "Ответ"]].map(([value, label]) => (
                <div key={label} className="p-5"><p className="text-xl font-semibold">{value}</p><p className="mt-1 text-[10px] text-[#828491]">{label}</p></div>
              ))}
            </div>
            <div className="border-t border-[#2b2d32] p-5 text-xs text-[#9fa1b1]">
              <p className="flex items-center gap-2"><Check className="size-4 text-success-fg" /> Контейнер активен всегда</p>
              <p className="mt-3 flex items-center gap-2"><Check className="size-4 text-success-fg" /> Webhook отвечает</p>
              <p className="mt-3 flex items-center gap-2"><Check className="size-4 text-success-fg" /> Последний backup 8 минут назад</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 pb-5 sm:px-8 sm:pb-8">
        <div data-graphite-shell className="mx-auto max-w-[1320px] rounded-[14px] bg-[#121519] px-6 py-20 text-center">
          <Rocket className="mx-auto size-7 text-[#4f81f7]" />
      ...
      <footer className="px-5 py-12 sm:px-8">
        <div className="mx-auto flex max-w-[1320px] flex-col gap-8 border-t border-[#2b2d32] pt-10 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3"><BrandMark /><span className="h-5 w-px bg-[#2b2d32]" /><span className="text-sm text-[#9fa1b1]">MAX Studio</span></div>
          <div className="flex flex-wrap gap-5 text-xs text-[#9fa1b1]">
