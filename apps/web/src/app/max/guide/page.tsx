        ...
        <div className="min-w-0">
          <GuideSection id="start" number="00" eyebrow="Перед началом" title="Что подготовить до первого клика" lead="MAX Studio выполняет сборку, backend, безопасное превью и публикацию. Для первого результата достаточно рабочей почты; MAX Partner и платёжный кабинет понадобятся только перед соответствующим запуском.">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2">
              {[
                ...
                [UserRoundCheck, "Сведения в приложении", "До production укажите оператора и контакты, которые увидит клиент. Это не повторная проверка MAX."],
          ...
          <GuideSection id="settings" number="04" eyebrow="Управляемая конфигурация" title="Данные приложения, каталог и юридические экраны" lead="Часть информации меняется без новой генерации и без расхода баланса. Это название, описание, контент, поддержка, данные оператора, юридические признаки и подтверждение URL в MAX.">
            <StepList steps={[
              ...
              { title: "Добавьте сведения для пользователей", text: <>Перед production-публикацией проверьте юридическое имя, регистрационные и контактные данные оператора. Это не повторная верификация MAX: сведения нужны внутри самого приложения по правилам площадки и законодательства.</>, result: "Пользователь может определить оператора и связаться с ним." },
          ...
          <GuideSection id="max-bot" number="06" eyebrow="Внешний кабинет MAX" title="Профиль MAX и безопасный запуск Mini App" lead="Профиль бизнеса и бот проверяет MAX Partner. Omnia не просит повторно вводить ИНН или ОГРН. До production секрет не нужен; перед запуском он один раз подключается для проверки signed initData, а после публикации URL вставляется в MAX Partner.">
            ...
            <StepList steps={[
              ...
              { title: "Подключите безопасный вход", text: <>До публикации откройте настройки MAX, вставьте секрет промодерированного бота и сохраните его через защищённую форму. ИНН и ОГРН повторно не заполняются.</>, result: "Сервер может проверить подпись initData и разделить данные пользователей." },
