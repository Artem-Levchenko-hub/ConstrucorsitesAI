...
function Kicker({ children }: { children: ReactNode }) {
  return <p className="omnia-kicker text-[#0381fa]">{children}</p>;
}
    ...
    <ol className="divide-y divide-[#25272b] overflow-hidden rounded-[12px] border border-[#2b2d32] bg-[#191b20]">
      {steps.map((step, index) => (
        <li key={step.title} className="grid gap-4 p-5 sm:grid-cols-[44px_minmax(0,1fr)] sm:p-6">
          <span className="grid size-8 place-items-center rounded-full border border-[#0381fa]/40 bg-[#0381fa]/[.06] font-mono text-[10px] font-semibold text-[#6a95fa]">
    ...
    <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((item, index) => (
        <div key={item} className="flex gap-3 rounded-[10px] border border-[#2b2d32] bg-[#191b20] p-4 text-xs leading-5 text-[#9fa1b1]">
          <span className="grid size-6 shrink-0 place-items-center rounded-full bg-[#0381fa] text-[10px] font-semibold text-[#121519]">{index + 1}</span>
          {item}
        </div>
      ))}
    </div>
      ...
      <section data-graphite-shell className="bg-[#121519] px-4 py-16 sm:px-7 sm:py-24 lg:py-28">
        <div className="mx-auto grid max-w-[1320px] gap-12 lg:grid-cols-[1fr_340px] lg:items-end">
          ...
          <div className="rounded-[14px] border border-white/14 bg-white/[.04] p-6">
            <div className="flex items-center gap-3"><BookOpenText className="size-5 text-[#0381fa]" /><p className="text-sm font-semibold">Как читать руководство</p></div>
            <ul className="mt-5 space-y-3 text-xs leading-5 text-white/48">
              ...
              <li>④ Секрет бота вводится только в защищённую форму перед production. Не передавайте его агенту.</li>
            </ul>
        ...
        <div className="min-w-0">
          <GuideSection id="start" number="00" eyebrow="Перед началом" title="Что подготовить до первого клика" lead="MAX Studio выполняет сборку, backend, безопасное превью и публикацию. Для первого результата достаточно рабочей почты; MAX Partner и платёжный кабинет понадобятся только перед соответствующим запуском.">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2">
              {[
                ...
                [UserRoundCheck, "Документы приложения", "Перед публикацией укажите название владельца и email поддержки для страниц приложения или дайте ссылку на свою политику. ИНН, ОГРН и телефон не нужны."],
                [Bot, "Безопасный вход — перед production", "Секрет промодерированного бота нужен серверу для проверки подписанного пользователя MAX."],
                ...
                return <article key={String(title)} className="rounded-[12px] border border-[#2b2d32] bg-[#191b20] p-5"><ItemIcon className="size-5 text-[#0381fa]" /><h3 className="mt-6 text-sm font-semibold">{String(title)}</h3><p className="mt-2 text-sm leading-6 text-[#9fa1b1]">{String(text)}</p></article>;
              })}
            </div>
          ...
          <GuideSection id="builder" number="03" eyebrow="Работа с агентом" title="Как получить рабочую сборку и не потерять контекст" lead="Редактор состоит из трёх зон: диалог с агентом, живое мобильное превью и мастер готовности к запуску. Все изменения сохраняются на сервере и попадают в историю проекта.">
            ...
            <div className="mt-4 grid gap-3">
              ...
              ].map(([label, prompt]) => <div key={label} className="rounded-[10px] border border-[#2b2d32] bg-[#191b20] p-4"><p className="font-mono text-[9px] uppercase tracking-[.14em] text-[#0381fa]">{label}</p><p className="mt-2 text-sm leading-6 text-[#9fa1b1]">{prompt}</p></div>)}
            </div>
          </GuideSection>
          ...
          <GuideSection id="integrations" number="05" eyebrow="Необязательно" title="Дополнительные сервисы" lead="CRM, аналитика, платежи и другие провайдеры подключаются только когда нужны сценарию. Они не блокируют первую генерацию и превью.">
            <IntegrationVisual />
            <NumberedLegend items={["Сначала получите и проверьте основное приложение.", "Подключайте только сервисы, для которых уже есть реальный пользовательский сценарий."]} />
            <StepList steps={[
              { title: "Выберите сервис", text: <>В окне публикации нажмите <strong>«Выбрать сервисы»</strong>. Каталог откроется поверх редактора и покажет возможности и статус каждого сервиса.</> },
          ...
          <GuideSection id="publish" number="07" eyebrow="Production" title="Публикация на постоянный HTTPS-адрес" lead="Публикация собирает неизменяемую версию, разворачивает контейнер, проверяет health endpoint и только затем переключает production. Неудачная версия не должна заменять работающую.">
            ...
            <StepList steps={[
              { title: "Откройте мастер", text: <>В верхней части редактора нажмите <strong>«Опубликовать»</strong>. Проверки, подключение MAX и публикация проходят в окнах поверх редактора.</>, result: "Виден следующий шаг. Закрытие окна возвращает к вашему чату." },
              { title: "Закройте readiness", text: <>Проверьте сборку, данные приложения, поддержку, юридические страницы и безопасный вход MAX. Отдельно настраивать webhook не требуется.</>, result: "Следующий шаг мастера — «Опубликуйте приложение»." },
          ...
          <GuideSection id="acceptance" number="09" eyebrow="Настоящий клиент MAX" title="Приёмка двумя разными пользователями" lead="Браузерное превью проверяет интерфейс, но не доказывает интеграцию с MAX. Финальная приёмка проводится в мобильном клиенте через кнопку настоящего промодерированного бота.">
            ...
            <div className="mt-6 rounded-[12px] border border-[#2b2d32] bg-[#121519] p-6 text-white">
              <p className="omnia-kicker text-[#0381fa]">Протокол приёмки</p>
              <div className="mt-6 grid gap-px overflow-hidden rounded-[10px] border border-white/12 bg-white/12 sm:grid-cols-3">
                {[
                  ...
                  [Link2, "Запуск по URL", "Кнопка в MAX открывает текущий production URL без промежуточного экрана."],
                ].map(([Icon, title, copy]) => { const ItemIcon = Icon as typeof Users; return <div key={String(title)} className="bg-[#121519] p-5"><ItemIcon className="size-5 text-[#0381fa]" /><p className="mt-7 text-sm font-semibold">{String(title)}</p><p className="mt-2 text-xs leading-5 text-white/45">{String(copy)}</p></div>; })}
              </div>
            </div>
            <StepList steps={[
              ...
              { title: "Запишите начальное состояние", text: <>Production имеет done, health зелёный, URL подтверждён. Не вносите изменения во время прогона.</> },
              ...
              { title: "Зафиксируйте результат", text: <>Сохраните версию, production URL, username бота и итог каждого пункта. Не сохраняйте initData или персональные идентификаторы в публичном отчёте.</>, result: "Два пользователя завершили путь, запуск и изоляция данных подтверждены." },
            ]} />
          </GuideSection>

          <GuideSection id="operations" number="10" eyebrow="Эксплуатация" title="Что делать после запуска и как выпускать изменения" lead="После запуска проект остаётся управляемым: видны runtime, health, production URL и история публикаций. Новое изменение проходит тот же цикл — правка, превью, новая версия, deploy и проверка.">
            <DashboardVisual />
