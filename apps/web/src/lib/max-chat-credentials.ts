...
export function redactChatSecrets(text: string): string {
  ...
  for (const pattern of SECRET_PATTERNS) {
    safe = safe.replace(new RegExp(pattern.source, pattern.flags), "[ключ сохранён в Omnia]");
  }
...
export function resolveChatCredential(
  ...
  const secrets = highConfidence.length ? highConfidence : fallback ? [fallback] : [];
  ...
  const secretFields = provider.fields.filter((field) => field.secret && field.required);
  const missingPublic = provider.fields.filter((field) => !field.secret && field.required);
  if (
    !provider.available ||
  ...
  const safePrompt = [
    ...
    "Не запрашивай и не записывай ключ в код. Используй управляемый integration runtime.",
    `Публичная документация: ${provider.docs_url}`,
  ]
    .filter(Boolean)
    .join("\n\n");
