...
it("refreshes the managed age marking from saved config on mount and return", async () => {
  ...
  const source = readFileSync(resolve(process.cwd(), "../orchestrator/templates/max-miniapp-nextjs/src/components/YleumCompliance.tsx"), "utf8");
  const compiled = ts.transpileModule(source, { compilerOptions: {
    module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022, jsx: ts.JsxEmit.ReactJSX,
  } }).outputText;
  const readConfig = vi.fn().mockResolvedValue({ legal: { age_rating: "12+" } });
  const exports: { YleumCompliance?: React.ComponentType } = {};
  new Function("exports", "require", compiled)(exports, (name: string) => {
    if (name === "react") return React;
    if (name === "react/jsx-runtime") return jsxRuntime;
    if (name.endsWith("integration-client")) return { getYleumAppConfig: readConfig };
  ...
  try {
    const Footer = exports.YleumCompliance!;
