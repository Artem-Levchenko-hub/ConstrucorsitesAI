...
describe("dark blue product theme", () => {
  ...
  /**
   * Единственное исключение из запрета оранжевого — файл с рисунками товаров на
   * витрине. Правило защищает ИНТЕРФЕЙС: оранжевый спорит с синим брендом и
   * грязнит тёмную тему. Но кофе коричневый, а круассан золотистый, и подгонять
   * еду под палитру значит нарисовать синий круассан. В файле нет ни одной
   * кнопки, рамки или подложки интерфейса — только <svg> самих предметов; если
   * туда попадёт элемент интерфейса, исключение надо отзывать, а не расширять.
   */
  const IMAGERY_ONLY = "components/marketing/landing/ProductArt.tsx";
