...
export function ProductComingSoon({
  ...
  const en = locale === "en";
  return (
    <div data-product-shell className="min-h-svh bg-[#121519] text-white antialiased">
      <header className="border-b border-[#2b2d32] bg-[#191b20]">
        <div className="mx-auto flex h-16 max-w-[1200px] items-center justify-between px-5 sm:px-8">
          <BrandMark />
          <div className="flex items-center gap-4"><div className="hidden sm:block"><LocaleSwitcher /></div><Link href="/" className="text-xs text-[#9fa1b1] hover:text-white">{en ? "All products" : "Все направления"}</Link></div>
        </div>
      </header>
      <main className="mx-auto grid min-h-[calc(100svh-65px)] max-w-[1440px] lg:grid-cols-[minmax(0,1.1fr)_minmax(400px,.9fr)]">
        <section className="flex flex-col px-5 py-12 sm:px-8 lg:border-r lg:border-[#2b2d32] lg:px-12 lg:py-16 xl:px-16">
          <div className="omnia-kicker flex items-center gap-2 text-[#4f81f7]"><Clock3 className="size-4" />{en ? "In development" : "В разработке"}</div>
          <div className="my-auto py-16">
            <span className="mb-8 grid size-12 place-items-center rounded-[8px] bg-[#2b2d32] text-[#4f81f7]"><Icon className="size-6" strokeWidth={1.5} /></span>
            <h1 className="max-w-4xl text-[clamp(48px,6.5vw,92px)] font-semibold leading-[.9] tracking-[-.06em]">{title}</h1>
            <p className="mt-8 max-w-2xl text-lg leading-8 text-[#9fa1b1]">{description}</p>
          </div>
          <div className="flex flex-wrap gap-3 border-t border-[#2b2d32] pt-6">
        ...
        <aside data-graphite-shell className="flex flex-col bg-[#121519] p-6 text-white sm:p-8 lg:p-10">
