...
export function MaxUsageBreakdown({ projectId }: { projectId: Uuid }) {
  ...
  return (
    <details className="group relative" data-testid="max-usage-breakdown">
      ...
      <section className="absolute right-0 top-11 z-[80] w-[340px] max-w-[calc(100vw-24px)] rounded-[12px] border border-[#d8d4cb] bg-[#fcfbf7] p-4 shadow-[0_24px_70px_rgba(23,23,22,.16)]">
        <div className="flex items-start justify-between gap-4 border-b border-[#e5e1d8] pb-3">
          <div>
            <p className="omnia-kicker text-[#8d887f]">Текущая сборка</p>
            <p className="mt-1 text-xl font-semibold tracking-[-.03em]">{rub(current)} ₽</p>
          </div>
          <div className="text-right text-[10px] text-[#8d887f]">
            <p>За всё время</p>
            <p className="mt-1 font-semibold text-[#494741]">{rub(usage.data?.total_cost_rub ?? 0)} ₽</p>
          </div>
        </div>

        {usage.isError ? (
          <p className="py-5 text-xs leading-5 text-[#8d887f]">Не удалось загрузить разбивку. Она обновится автоматически.</p>
        ) : (
          <div className="mt-3 space-y-2">
            {(usage.data?.stages ?? []).map((stage) => (
              <div key={stage.id} className="rounded-[9px] border border-[#e5e1d8] bg-white p-3">
                ...
                <p className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[9px] text-[#8d887f]">
