...
export function MaxWorkspaceShell({
  ...
  return (
    <div data-light-shell className="grid h-dvh min-h-0 grid-cols-1 overflow-hidden bg-[#f5f3ee] text-[#171716] lg:grid-cols-[220px_minmax(0,1fr)] xl:grid-cols-[220px_minmax(420px,1fr)_360px]">
      <aside
      ...
      >
        <div className="flex h-16 items-center justify-between border-b border-[#d8d4cb] px-5">
          <BrandMark href="/max" />
          <button type="button" onClick={() => setMobileNavOpen(false)} className="rounded-[8px] p-2 text-[#8d887f] lg:hidden" aria-label="Закрыть меню">
            <X className="size-4" />
          </button>
        </div>

        <div className="min-h-0 flex-1 overflow-y-auto p-3">
          <Link href="/max" className="flex h-9 items-center gap-3 rounded-[8px] px-3 text-xs text-[#6d6962] hover:bg-[#f5f3ee]">
          ...
          <nav className="mt-2 space-y-1">
            {maxProjects.map((item) => {
              ...
              return (
                <Link key={item.id} href={`/max/${item.id}`} className={`flex h-9 items-center gap-3 rounded-[8px] px-3 text-xs transition ${active ? "bg-[#ece8df] font-medium" : "text-[#6d6962] hover:bg-[#f5f3ee]"}`}>
          ...
          <nav className="mt-2 space-y-1">
            <Link href={`/max/${project.id}/integrations`} className="flex h-9 items-center gap-3 rounded-[8px] px-3 text-xs text-[#6d6962] hover:bg-[#f5f3ee]">
              <Plug className="size-4" /> Интеграции
            </Link>
            <Link href={`/max/${project.id}/settings`} className="flex h-9 items-center gap-3 rounded-[8px] px-3 text-xs text-[#6d6962] hover:bg-[#f5f3ee]">
              <Bot className="size-4" /> MAX и приложение
            </Link>
            <Link href={`/max/${project.id}/publish`} className="flex h-9 items-center gap-3 rounded-[8px] px-3 text-xs text-[#6d6962] hover:bg-[#f5f3ee]">
              <Rocket className="size-4" /> Публикация
            </Link>
            <Link href={`/max/${project.id}/dashboard`} className="flex h-9 items-center gap-3 rounded-[8px] px-3 text-xs text-[#6d6962] hover:bg-[#f5f3ee]">
        ...
        <div className="border-t border-[#d8d4cb] p-3">
          <Link href="/account" className="flex min-w-0 items-center gap-2.5 rounded-[8px] p-2 hover:bg-[#f5f3ee]">
          ...
          <form action={logoutAction} className="mt-1">
            <button type="submit" className="flex w-full items-center gap-2 rounded-[8px] px-2 py-1.5 text-[10px] text-[#8d887f] hover:bg-[#f5f3ee]"><LogOut className="size-3.5" />Выйти</button>
          </form>
        </div>
      </aside>

      <section className="flex min-h-0 min-w-0 flex-col border-r border-[#d8d4cb] bg-[#fcfbf7]">
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-[#d8d4cb] px-4 sm:px-5">
          <div className="flex min-w-0 items-center gap-3">
            <button type="button" onClick={() => setMobileNavOpen(true)} className="rounded-[8px] p-2 text-[#6d6962] lg:hidden" aria-label="Открыть меню"><Menu className="size-4" /></button>
          ...
          <div className="flex items-center gap-2">
            <Link href={`/max/${project.id}/integrations`} className="hidden h-9 items-center rounded-[8px] border border-[#d8d4cb] px-3 text-xs text-[#6d6962] hover:bg-[#f5f3ee] sm:inline-flex">Интеграции</Link>
            <button type="button" onClick={() => setLaunchOpen(true)} className="inline-flex h-9 items-center gap-2 rounded-[8px] bg-[#f15a38] px-4 text-xs font-semibold text-white hover:bg-[#d94929]">
              Опубликовать <ChevronDown className="size-3.5" />
            </button>
          </div>
        </header>
