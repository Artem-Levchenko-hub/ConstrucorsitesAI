...
function TargetRow({
  ...
  return (
    <div
      className={`rounded-lg border p-2.5 ${selected ? "border-[#3b82f6] bg-[rgba(49,91,215,0.1)]" : "border-border-subtle"}`}
    >
      <div className="flex items-center gap-2">
        ...
        >
          <span
            className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full border ${selected ? "border-[#3b82f6] bg-[#3b82f6]" : "border-border-strong"}`}
          >
    ...
    <div className="rounded-lg border border-border-subtle p-3 space-y-2.5">
      ...
      <div>
        ...
        <div className="flex gap-1.5">
          <button type="button" onClick={() => setAuthType("key")} className={`flex-1 rounded-md border px-2 py-1.5 text-xs ${authType === "key" ? "border-[#3b82f6] bg-[rgba(49,91,215,0.1)]" : "border-border-subtle text-fg-secondary"}`}>
            SSH-ключ (безопаснее)
          </button>
          <button type="button" onClick={() => setAuthType("password")} className={`flex-1 rounded-md border px-2 py-1.5 text-xs ${authType === "password" ? "border-[#3b82f6] bg-[rgba(49,91,215,0.1)]" : "border-border-subtle text-fg-secondary"}`}>
            Логин + пароль
          </button>
        </div>
      </div>
...
function CopyValue({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-2 rounded-md bg-surface-overlay px-2 py-1.5">
      ...
      >
        {copied ? <Check className="h-3.5 w-3.5 text-[#8b5cf6]" /> : <Copy className="h-3.5 w-3.5" />}
      </button>
    </div>
  );
}
  ...
  return (
    <div className="rounded-lg border border-border-subtle p-3 space-y-2.5">
      <div className="flex items-center gap-2">
        ...
        {certActive ? (
          <a href={`https://${domain.host}`} target="_blank" rel="noreferrer" className="text-xs text-[#8b5cf6] hover:underline flex items-center gap-1">
      ...
      {certActive ? (
        ...
        <>
          ...
          <div className="rounded-md border border-[#3b82f6]/30 bg-[rgba(49,91,215,0.1)] p-2">
