...
- Intended target: push reviewed HEAD to `origin/main`, then rebuild/restart only `web` from compose project `full` at `/opt/omnia/apps/llm-gateway/deploy/full`.
- Source revision, GitHub push and new web image: pending controller verification.
- New `/web-health` revision, HTTP endpoint checks and web container health: pending controller verification.
- API, worker and gateway container identity preservation: pending controller verification.
- Report alias `/var/www/otchet/data.json`: publish only after the source deployment is verified.
