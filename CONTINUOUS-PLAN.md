...
**Шаг 1 — lock (один поток).** Файл `.claude/routine.lock`.
- Существует И моложе 45 мин → активная итерация идёт → выйти молча.
- Иначе записать туда текущий UTC-таймстамп (это твой lock). **Пере-штампуй lock после каждой задачи** (чтобы длинная итерация не выглядела протухшей). В конце итерации (успех ИЛИ ошибка) — **удалить** lock.
...
**Шаг 4 — выбор задачи.** Первый `[ ]` сверху вниз по ROADMAP. Один маленький связный слайс (~1–3 файла). Крупная — разбить, сделать первый под-слайс.
