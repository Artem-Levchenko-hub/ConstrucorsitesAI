...
6. **Прибраться (resource-guard):** после E2E аппа — останови/снеси созданный тест-контейнер; не копить (беречь сервер, раздел 0 п.8).
7. **Точечные/ручные правки — проверять ЧЕРЕЗ КЛИКЕР (select-mode), не только API.** Если задача трогает правки UI: клик по элементу в превью → правка реально применилась в превью, без слома/дрейфа; ручная правка (цвет/шрифт/текст) + Save снапшотит. См. Phase 2.5.
